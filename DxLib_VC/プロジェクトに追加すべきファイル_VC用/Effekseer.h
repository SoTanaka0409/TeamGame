...
#define ES_SAFE_ADDREF(val)                                                    \
  static_assert(std::is_class<decltype(val)>::value != true,                   \
                "val must not be class/struct");                               \
  if ((val) != nullptr) {                                                      \
    (val)->AddRef();                                                           \
  }
...
*/
inline int32_t ConvertUtf16ToUtf8(char *dst, int32_t dst_size,
                                  const char16_t *src) {
  int32_t cnt = 0;
...
/**
    ...
    @return    length except 0
*/
inline int32_t ConvertUtf8ToUtf16(char16_t *dst, int32_t dst_size,
                                  const char *src) {
  int32_t i, code = 0;
  int8_t c0, c1, c2 = 0;
...
public:
  ...
  virtual int Release() {
    bool destroy = std::atomic_fetch_sub_explicit(
                       &m_reference, 1, std::memory_order_consume) == 1;
    if (destroy) {
      delete this;
      return 0;
    }
...
template <typename T>
inline std::unique_ptr<T, ReferenceDeleter<T>>
CreateUniqueReference(T *ptr, bool addRef = false) {
  if (ptr == nullptr)
...
template <typename T> inline void SafeAddRef(T *val) {
  if (val != nullptr) {
    val->AddRef();
  }
}

template <typename T> inline void SafeRelease(T *&val) {
  if (val != nullptr) {
    val->Release();
    val = nullptr;
  }
...
public:
  ...
  template <class U> void operator=(const RefPtr<U> &o) {
    auto ptr = o.Get();
    SafeAddRef(ptr);
    SafeRelease(ptr_);
    ptr_ = ptr;
  ...
  template <class U> void operator=(RefPtr<U> &&o) {
    auto ptr = o.Get();
    o.ptr_ = nullptr;
    SafeRelease(ptr_);
    ptr_ = ptr;
  ...
  template <class U> RefPtr(const RefPtr<U> &o) {
    auto ptr = o.Get();
    SafeAddRef(ptr);
    SafeRelease(ptr_);
    ptr_ = ptr;
  ...
  template <class U> RefPtr(RefPtr<U> &&o) {
    auto ptr = o.Get();
    o.ptr_ = nullptr;
    SafeRelease(ptr_);
    ptr_ = ptr;
  ...
  template <class U> RefPtr<U> DownCast() const {
    auto ptr = Get();
    SafeAddRef(ptr);
...
template <class T, class... Arg> RefPtr<T> MakeRefPtr(Arg &&...args) {
...
/**
        @brief	\~english	Parameters about a depth which is passed into a
   renderer
                        \~japanese
   レンダラーに渡されるデプスに関するパラメーター
*/
...
/**
        @brief	\~english	Flipbook parameter parameters which is passed
   into a renderer
                        \~japanese
   レンダラーに渡されるフリップブックに関するパラメーター
*/
...
/**
        @brief	\~english	Common parameters which is passed into a
   renderer
                        \~japanese
   レンダラーに渡される共通に関するパラメーター
*/
...
public:
  ...
  virtual bool Equal(const RenderingUserData *rhs) const { return true; }
};
...
template <class T> struct CustomAllocator {
  ...
  T *allocate(std::size_t n) {
    return reinterpret_cast<T *>(
        GetMallocFunc()(sizeof(T) * static_cast<uint32_t>(n)));
  }
...
template <class T> struct CustomAlignedAllocator {
  ...
  T *allocate(std::size_t n) {
    return reinterpret_cast<T *>(
        GetAlignedMallocFunc()(sizeof(T) * static_cast<uint32_t>(n), 16));
  }
  void deallocate(T *p, std::size_t n) {
    GetAlignedFreeFunc()(p, sizeof(T) * static_cast<uint32_t>(n));
  }

  bool operator==(const CustomAlignedAllocator<T> &) { return true; }

  bool operator!=(const CustomAlignedAllocator<T> &) { return false; }
};
...
template <class T>
using CustomString =
    std::basic_string<T, std::char_traits<T>, CustomAllocator<T>>;
...
template <class T, class U>
using CustomMap =
    std::map<T, U, std::less<T>, CustomAllocator<std::pair<const T, U>>>;
template <class T, class U>
using CustomAlignedMap =
    std::map<T, U, std::less<T>, CustomAlignedAllocator<std::pair<const T, U>>>;
template <class T, class U, class Hasher = std::hash<T>,
          class KeyEq = std::equal_to<T>>
using CustomUnorderedMap =
    std::unordered_map<T, U, Hasher, KeyEq,
                       CustomAllocator<std::pair<const T, U>>>;
template <class T, class U, class Hasher = std::hash<T>,
          class KeyEq = std::equal_to<T>>
using CustomAlignedUnorderedMap =
    std::unordered_map<T, U, Hasher, KeyEq,
                       CustomAlignedAllocator<std::pair<const T, U>>>;
...
public:
  ...
  StringView(const T *ptr) : ptr_(ptr), size_(Traits::length(ptr)) {}

  StringView(const T *ptr, size_t size) : ptr_(ptr), size_(size) {}

  template <size_t N> StringView(const T ptr[N]) : ptr_(ptr), size_(N) {}
  ...
  bool operator==(const StringView<T> &rhs) const {
    return size() == rhs.size() &&
           Traits::compare(data(), rhs.data(), size()) == 0;
  }
...
public:
  ...
  */
  ...
  static Vector3D &Transform(Vector3D &o, const Vector3D &in,
                             const Matrix43 &mat);

  static Vector3D &Transform(Vector3D &o, const Vector3D &in,
                             const Matrix44 &mat);

  static Vector3D &TransformWithW(Vector3D &o, const Vector3D &in,
                                  const Matrix44 &mat);
...
public:
  ...
  */
  ...
  /**
          @brief	check whether all values are not valid number(not nan,
     not inf)
  */
  bool IsValid() const;
...
public:
  ...
  */
  ...
  /**
          @brief	カメラ行列化(右手系)
  */
  Matrix44 &LookAtRH(const Vector3D &eye, const Vector3D &at,
                     const Vector3D &up);

  /**
          @brief	カメラ行列化(左手系)
  */
  Matrix44 &LookAtLH(const Vector3D &eye, const Vector3D &at,
                     const Vector3D &up);
  ...
  */
  Matrix44 &PerspectiveFovRH_OpenGL(float ovY, float aspect, float zn,
                                    float zf);
  ...
  */
   ...
   */
  Matrix44 &PerspectiveFovLH_OpenGL(float ovY, float aspect, float zn,
                                    float zf);
...
public:
  ...
  /**
          @brief
          \~English	try to open a reader. It need not to succeeds in opening
     it.
...
inline bool IsDepthTextureFormat(TextureFormatType format) {
  return format == TextureFormatType::D24S8 ||
         format == TextureFormatType::D32S8 || format == TextureFormatType::D32;
}
...
/**
        ...
        Only for OpenGL
*/
class UniformLayout : public ReferenceObject {
private:
  CustomVector<CustomString<char>> textures_;
  CustomVector<UniformLayoutElement> elements_;

public:
  UniformLayout(CustomVector<CustomString<char>> textures,
                CustomVector<UniformLayoutElement> elements)
      : textures_(std::move(textures)), elements_(std::move(elements)) {}
...
public:
  ...
  IndexBufferStrideType GetStrideType() const { return strideType_; }
...
class UniformBuffer : public ReferenceObject {
public:
...
inline TextureUsageType operator|(TextureUsageType lhs, TextureUsageType rhs) {
  return static_cast<TextureUsageType>(static_cast<uint32_t>(lhs) |
...
public:
  ...
  TextureParameter GetParameter() const { return param_; }
};
...
class FrameBuffer : public ReferenceObject {
public:
  FrameBuffer() = default;
...
public:
  ...
  /**
          ...
          @param	initialData	the initial data of buffer. If it is
     null, not initialized.
          @param	isDynamic	whether is the buffer dynamic? (for
     DirectX9, 11 or OpenGL)
          @return	VertexBuffer
  */
  virtual VertexBufferRef
  CreateVertexBuffer(int32_t size, const void *initialData, bool isDynamic) {
  ...
  /**
     ...
     null, not initialized.
          ...
          @return	IndexBuffer
  */
  virtual IndexBufferRef CreateIndexBuffer(int32_t elementCount,
                                           const void *initialData,
                                           IndexBufferStrideType stride) {
  ...
  */
  virtual bool UpdateVertexBuffer(VertexBufferRef &buffer, int32_t size,
                                  int32_t offset, const void *data) {
    return false;
  }
  ...
  /**
          ...
          @return	Succeeded in updating?
  */
  virtual bool UpdateIndexBuffer(IndexBufferRef &buffer, int32_t size,
                                 int32_t offset, const void *data) {
    return false;
  }
  ...
  /**
          ...
          @return	Succeeded in updating?
  */
  virtual bool UpdateUniformBuffer(UniformBufferRef &buffer, int32_t size,
                                   int32_t offset, const void *data) {
    return false;
  }
  ...
  /**
          ...
          @param	elements	a pointer of array of vertex layout
     elements
          @param	elementCount	the number of elements
  */
  virtual VertexLayoutRef
  CreateVertexLayout(const VertexLayoutElement *elements,
  ...
  */
  virtual UniformBufferRef CreateUniformBuffer(int32_t size,
                                               const void *initialData) {
    return UniformBufferRef{};
  }

  virtual PipelineStateRef
  CreatePipelineState(const PipelineStateParameter &param) {
    return PipelineStateRef{};
  }

  virtual FrameBufferRef CreateFrameBuffer(const TextureFormatType *formats,
                                           int32_t formatCount,
                                           TextureFormatType depthFormat) {
    return FrameBufferRef{};
  }

  virtual RenderPassRef
  CreateRenderPass(FixedSizeVector<TextureRef, RenderTargetMax> &textures,
  ...
  virtual TextureRef CreateTexture(
      ...
      const CustomVector<uint8_t> &initialData = CustomVector<uint8_t>()) {
    return TextureRef{};
  }
  ...
  virtual bool CopyTexture(TextureRef &dst, TextureRef &src,
                           const std::array<int, 3> &dstPos,
                           const std::array<int, 3> &srcPos,
                           const std::array<int, 3> &size, int32_t dstLayer,
                           int32_t srcLayer) {
    return false;
  }
  ...
  /**
          ...
          @return	Shader
  */
  virtual ShaderRef CreateShaderFromKey(const char *key) { return ShaderRef{}; }

  virtual ShaderRef
  CreateShaderFromCodes(const CustomVector<StringView<char>> &vsCodes,
                        const CustomVector<StringView<char>> &psCodes,
                        UniformLayoutRef layout = nullptr) {
    return ShaderRef{};
  }

  virtual ShaderRef CreateShaderFromBinary(const void *vsData,
                                           int32_t vsDataSize,
                                           const void *psData,
                                           int32_t psDataSize) {
    return ShaderRef{};
  ...
  */
  // virtual ComputeBuffer* CreateComputeBuffer(int32_t size, const void*
  // initialData)
  // {
  // 	return nullptr;
  // }
  ...
  virtual void BeginRenderPass(RenderPassRef &renderPass, bool isColorCleared,
                               bool isDepthCleared, Color clearColor) {}
...
inline int32_t GetVertexLayoutFormatSize(VertexLayoutFormat format) {
  int32_t size = 0;
  if (format == Effekseer::Backend::VertexLayoutFormat::R8G8B8A8_UINT ||
      format == Effekseer::Backend::VertexLayoutFormat::R8G8B8A8_UNORM) {
    size = 4;
    ...
    size = sizeof(float) * 2;
             ...
             Effekseer::Backend::VertexLayoutFormat::R32G32B32_FLOAT) {
    size = sizeof(float) * 3;
  } else if (format ==
             Effekseer::Backend::VertexLayoutFormat::R32G32B32A32_FLOAT) {
    size = sizeof(float) * 4;
  } else {
    assert(0);
  }
...
public:
  ...
  const CustomString<char16_t> &GetPath() { return path_; }
...
public:
  ...
  int32_t GetWidth() const { return backend_->GetParameter().Size[0]; }
  int32_t GetHeight() const { return backend_->GetParameter().Size[1]; }
...
/**
        @brief
        \~English A class to edit an instance of EffectParameter for supporting
   original format when a binary is loaded.
        \~Japanese
   独自フォーマットをサポートするための、バイナリが読み込まれた時にEffectParameterのインスタンスを編集するクラス
*/
...
public:
  ...
  */
  bool LoadBody(Effect *effect, const void *data, int32_t size,
                float magnification, const char16_t *materialPath);
  ...
  */
  void SetTexture(Effect *effect, int32_t index, TextureType type,
                  TextureRef data);
  ...
  */
  ...
  /**
          @brief
          \~English this method is called to check whether loaded binary are
     supported.
          \~Japanese
     バイナリがサポートされているか確認するためにこのメソッドが呼ばれる。
  */
  virtual bool OnCheckIsBinarySupported(const void *data, int32_t size);

  /**
          @brief
          \~English this method is called to check whether reloading are
     supported.
          \~Japanese
     リロードがサポートされているか確認するためにこのメソッドが呼ばれる。
  */
  ...
  /**
          ...
          \~Japanese
     バイナリからエフェクトを読み込む時に、このメソッドが呼ばれる。
  */
  virtual bool OnLoading(Effect *effect, const void *data, int32_t size,
                         float magnification, const char16_t *materialPath);
  ...
  */
  virtual void OnLoadingResource(Effect *effect, const void *data, int32_t size,
  ...
  */
  ...
  /**
  \~English get whether resources are loaded automatically when a binary is
  loaded
...
public:
  ...
  */
  static EffectRef Create(const ManagerRef &manager, const void *data,
                          int32_t size, float magnification = 1.0f,
  ...
  /**
          ...
          @return	エフェクト。失敗した場合はnullptrを返す。
  */
  static EffectRef Create(const ManagerRef &manager, const char16_t *path,
  ...
  @return	エフェクト。失敗した場合はnullptrを返す。
*/
  static EffectRef Create(const SettingRef &setting, const void *data,
                          int32_t size, float magnification = 1.0f,
                          const char16_t *materialPath = nullptr);
  ...
  /**
          ...
          @return	エフェクト。失敗した場合はnullptrを返す。
  */
  static EffectRef Create(const SettingRef &setting, const char16_t *path,
                          float magnification = 1.0f,
                          const char16_t *materialPath = nullptr);
  ...
  */
  static ::Effekseer::EffectLoaderRef
  CreateEffectLoader(::Effekseer::FileInterfaceRef fileInterface = nullptr);

  /**
  @brief
  \~English	Get this effect's name. If this effect is loaded from file,
  default name is file name without extention.
  \~Japanese
  エフェクトの名前を取得する。もしファイルからエフェクトを読み込んだ場合、名前は拡張子を除いたファイル名である。
  */
  ...
  */
  ...
  /**
  @brief	\~English	Get the magnification multiplied by the
  magnification at the time of loaded and exported.
                  \~Japanese
  ...
  */
  ...
  /**
          @brief
          \~English	Get loading parameter supecfied by EffectFactory. This
     parameter is not used unless EffectFactory is used
          \~Japanese
  ...
  */
  ...
  /**
  @brief	\~English	Get the number of stored procedural model's
  pointer
  ...
  */
  virtual const ProceduralModelParameter *
  GetProceduralModelParameter(int n) const = 0;
  ...
  */
  ...
  /**
          ...
          @note
          \~English
          If an effect is generated with Setting, the effect in managers is
     reloaded with managers If reloadingThreadType is RenderThread, new
     resources aren't loaded and old resources aren't disposed.
          \~Japanese
          Settingを用いてエフェクトを生成したときに、Managerを指定することで対象のManager内のエフェクトのリロードを行う。
          もし、reloadingThreadType が
     RenderThreadの場合、新規のリソースは読み込まれず、古いリソースは破棄されない。
  */
  virtual bool Reload(
      ManagerRef *managers, int32_t managersCount, const void *data,
      int32_t size, const char16_t *materialPath = nullptr,
  ...
  */
  virtual bool Reload(
      ManagerRef *managers, int32_t managersCount, const char16_t *path,
      const char16_t *materialPath = nullptr,
      ReloadingThreadType reloadingThreadType = ReloadingThreadType::Main) = 0;
  ...
  */
  virtual void ReloadResources(const void *data = nullptr, int32_t size = 0,
                               const char16_t *materialPath = nullptr) = 0;
...
public:
  ...
  */
  ...
  /**
          @brief
          \~English	Get a generation in the node tree. The generation
     increases by 1 as it moves a child node.
          \~Japanese
     ノードツリーの世代を取得する。世代は子のノードになるにしたがって1増える。
  */
  ...
  */
  virtual EffectInstanceTerm
  CalculateInstanceTerm(EffectInstanceTerm &parentTerm) const = 0;
  ...
  */
  ...
  /**
          ...
          The variable is compared by the comparison function described by the
     inheritance of RenderingUserData, and if the values are different, DrawCall
     is issued.
          \~Japanese
  ...
  */
  virtual void
  SetRenderingUserData(const RefPtr<RenderingUserData> &renderingUserData) = 0;
};
...
/**
        ...
        isRemovingManager Whether the effect instance is removed when the
   manager is removed.
        \~Japanese
        manager 所属しているマネージャー
        handle エフェクトのインスタンスのハンドル
        isRemovingManager
   マネージャーを破棄したときにエフェクトのインスタンスを破棄しているか
*/
using EffectInstanceRemovingCallback =
    std::function<void(Manager *, Handle, bool)>;
...
public:
  ...
  struct UpdateParameter {
    ...
    */
    ...
    /**
            ...
            \~Japanese 更新間隔
            @note
            \~English For example, DeltaTime is 2 and UpdateInterval is 1, an
       effect is update twice
            \~Japanese
       例えば、DeltaTimeが2でUpdateIntervalが1の場合、エフェクトは2回更新される。
    */
    ...
    /**
            ...
            \~Japanese 同期更新を行う
            @note
            \~English If true, update processing is performed synchronously. If
       false, update processing is performed asynchronously (after this, do not
       call anything other than Draw)
            \~Japanese
       trueなら同期的に更新処理を行う。falseなら非同期的に更新処理を行う（次はDraw以外呼び出してはいけない）
    */
  ...
  */
  struct DrawParameter {
    ...
    */
    ...
    /**
            ...
            @note
            \~English For example, if effect's layer is 1 and
       CameraCullingMask's first bit is 1, this effect is shown.
            \~Japanese
    ...
    */
    ...
    /**
            @brief
            \~English Whether effects should be sorted by camera position and
       direction
  ...
  /**
          @brief
          \~English Parameters of Manager::SetLayerParameter to be set for each
     layer index.
          \~Japanese
     Manager::SetLayerParameterにレイヤーごとに設定するパラメーター
  */
  struct LayerParameter {
    /**
            @brief
            \~English
            Position of effects viewer to calculate distance of Level of Details
       system. Normally should be set the same position which is passed in
       translation of camera matrix.
            \~Japanese
            LODシステムで使用される視点の位置。
            通常はカメラの位置と同じ値を指定する。
    */
    ...
    /**
            @brief
            \~English
            Adds given value to calculated distance from viewer which is used
       for LOD selection. Useful for LODs debugging.
            \~Japanese
...
public:
  /**
          ...
          @param	autoFlip		[in]
     自動でスレッド間のデータを入れ替えるかどうか、を指定する。trueの場合、Update時に入れ替わる。
          @return	マネージャー
  */
  ...
  */
  ...
  /**
          @brief
          \~English Get a thread handle (HANDLE(win32), pthread_t(posix) or
     etc.)
          \~Japanese スレッドハンドルを取得する。(HANDLE(win32) や
     pthread_t(posix) など)
  */
  ...
  */
  ...
  /**
          ...
          This means that the number of used instances added resting resting
     instances is larger than the number of allocated onces by the number of
     root.
          \~Japanese
          Rootも個数に含まれる。つまり、Root削除をしていない限り、
  ...
  */
  ...
  /**
          ...
          \~Japanese	レイヤーパラメータ
   */
  virtual void SetLayerParameter(int32_t layer,
                                 const LayerParameter &layerParameter) = 0;
  ...
  */
  ...
  /**
     ...
     エフェクトのインスタンスの任意軸周りの反時計周りの回転角度を指定する。
          ...
          @param	angle	[in]	角度(ラジアン)
  */
  virtual void SetRotation(Handle handle, const Vector3D &axis,
                           float angle) = 0;
  ...
  */
  ...
  /**
          @brief
          \~English get a dynamic parameter, which changes effect parameters
     dynamically while playing
          \~Japanese
  ...
  */
  ...
  /**
          @brief
          \~English specfiy a dynamic parameter, which changes effect parameters
     dynamically while playing
          \~Japanese
  ...
  */
  virtual void SetRemovingCallback(Handle handle,
                                   EffectInstanceRemovingCallback callback) = 0;

  /**
  @brief	\~English	Get status that a particle of effect specified
  is shown.
  ...
  */
  ...
  /**
          @brief	\~English	Pause or resume a particle of effect
     specified.
          \~Japanese
  ...
  */
  ...
  /**
                  @brief	\~English	Pause or resume all particle of
     effects.
                  \~Japanese
  ...
  */
  ...
  /**
          @brief Stops new particles spawning but continues simulation of
  ...
  /**
          ...
          @note
          \~English For example, if effect's layer is 1 and CameraCullingMask's
     first bit is 1, this effect is shown.
          \~Japanese
     例えば、エフェクトのレイヤーが0でカリングマスクの最初のビットが1のときエフェクトは表示される。
  */
  ...
  */
  ...
  /**
          @brief
          \~English	Specify a rate of scale in relation to manager's time by
     a group.
          \~Japanese
     グループごとにマネージャーに対する時間の拡大率を設定する。
  */
  ...
  */
  ...
  /**
          @brief
          \~English	Specify a user pointer for custom renderer and custom
     sound player
          \~Japanese
     ハンドルごとにカスタムレンダラーやカスタムサウンド向けにユーザーポインタを設定する。
  */
  ...
  /**
          ...
          \~Japanese
     ハンドルごとにエフェクトのデフォルトランダムシード値を設定する。
  */
  ...
  */
  ...
  /**
          ...
          @note
          \~English
          You need to call BeginUpdate before starting update and EndUpdate
     after stopping update.
          \~Japanese
  ...
  */
  ...
  \~Japanese	描画処理を行う。
  */
  virtual void Draw(const Manager::DrawParameter &drawParameter =
                        Manager::DrawParameter()) = 0;
  ...
  \~Japanese	背面の描画処理を行う。
  */
  virtual void DrawBack(const Manager::DrawParameter &drawParameter =
                            Manager::DrawParameter()) = 0;
  ...
  \~Japanese	前面の描画処理を行う。
  */
  virtual void DrawFront(const Manager::DrawParameter &drawParameter =
                             Manager::DrawParameter()) = 0;
  ...
  */
  virtual void DrawHandle(Handle handle,
  ...
  */
  virtual void DrawHandleBack(Handle handle,
                              const Manager::DrawParameter &drawParameter =
                                  Manager::DrawParameter()) = 0;
  ...
  */
  virtual void DrawHandleFront(Handle handle,
                               const Manager::DrawParameter &drawParameter =
                                   Manager::DrawParameter()) = 0;
  ...
  */
  virtual bool GetIsCulled(Handle handle,
                           const Manager::DrawParameter &drawParameter) = 0;
  ...
  */
  virtual Handle Play(const EffectRef &effect, const Vector3D &position,
                      int32_t startFrame = 0) = 0;

  /**
          @brief
          \~English	Get a camera's culling mask to show all effects
          \~Japanese
     全てのエフェクトを表示するためのカメラのカリングマスクを取得する。
  */
...
/**
        ...
        \~Japanese
   エディタといったクライアントからエフェクトを編集するためのサーバー
*/
...
public:
  ...
  */
  virtual void Update(
      ManagerRef *managers = nullptr, int32_t managerCount = 0,
      ReloadingThreadType reloadingThreadType = ReloadingThreadType::Main) = 0;
...
public:
  ...
  virtual void Reload(ManagerRef manager, const char16_t *path,
                      const char16_t *key) = 0;
