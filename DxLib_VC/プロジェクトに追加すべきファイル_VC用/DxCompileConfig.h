...
// DxLib.h に書かれている using namespace DxLib ;
// を使用しない場合は次のコメントを外してください
...
// ( ＤＸアーカイブを無効にすると、ＤＸアーカイブを内部で使っている関係上
// DX_NON_MODEL と DX_NON_FILTER と DX_NON_MOVIE と DX_NON_NORMAL_DRAW_SHADER
// も有効になります )
// #define DX_NON_DXA
...
// 標準 WAVE
// ファイル以外の音声ファイルを使用しない方は次のコメントを外してください
// #define DX_NON_ACM

// DirectShow を使用した MP3
// ファイルのデコードを行わない場合は次のコメントをはずしてください
// #define DX_NON_DSHOW_MP3

// DirectShow
// を使用した動画ファイルのデコードを行わない場合は次のコメントをはずしてください
...
// ＦＢＸファイルを読み込む機能を使用する方は次のコメントを外してください(
// 使用には FBX SDK のセットアップが必要です )
...
// サウンド再生機能( ソフトウエアサウンド、MIDI含む
// )を無効にする場合は次のコメントを外して下さい
// #define DX_NON_SOUND
...
// 各ハンドルのエラーチェックを無効にする場合は次のコメントを外してください(
// 若干高速化される代わりに無効なハンドルを関数に渡すと即不正なメモリアクセスエラーが発生するようになります
// )
// #define DX_NON_HANDLE_ERROR_CHECK

// Direct3D11 を使用しない場合は以下のコメントを外してください(
// 現在開発中なので、必ずコメントを外した状態にしてください )
...
#if defined(DX_GCC_COMPILE) || defined(__ANDROID__) || defined(__APPLE__) ||   \
    defined(BC2_COMPILER)
#define USE_ULL
#endif
