...
/**
        @brief	\~english	uv texture parameters which is passed into a
   renderer
                        \~japanese
   レンダラーに渡されるUVTextureに関するパラメーター
*/
...
public:
  ...
  */
  virtual TextureRef Load(const void *data, int32_t size,
                          TextureType textureType, bool isMipMapEnabled) {
    return nullptr;
  }
...
public:
  ...
  Emitter GetEmitter(IRandObject *g, int32_t time, CoordinateSystem coordinate,
                     float magnification);

  Emitter GetEmitterFromVertex(IRandObject *g, int32_t time,
                               CoordinateSystem coordinate,
                               float magnification);

  Emitter GetEmitterFromVertex(int32_t index, int32_t time,
                               CoordinateSystem coordinate,
                               float magnification);

  Emitter GetEmitterFromFace(IRandObject *g, int32_t time,
                             CoordinateSystem coordinate, float magnification);

  Emitter GetEmitterFromFace(int32_t index, int32_t time,
                             CoordinateSystem coordinate, float magnification);

  bool StoreBufferToGPU(Backend::GraphicsDevice *graphicsDevice,
                        bool flipVertexColor = false);
...
public:
  ...
  */
  virtual MaterialRef Load(const char16_t *path) { return nullptr; }
  ...
  /**
          ...
          \~Japanese	読み込まれたマテリアルのポインタ
  */
  virtual MaterialRef Load(const void *data, int32_t size,
...
private:
   ...
   * 戻り値 : 計算結果
   */
  double CalcBSplineBasisFunc(const std::vector<double> &knot, unsigned int j,
                              unsigned int p, double t) {
    if (knot.size() == 0)
    ...
    // 漸化式の計算
    double d1 = (knot[j + p] == knot[j])
                    ? 0
                    : (t - knot[j]) * CalcBSplineBasisFunc(knot, j, p - 1, t) /
                          (knot[j + p] - knot[j]);
    double d2 = (knot[j + p + 1] == knot[j + 1])
                    ? 0
                    : (knot[j + p + 1] - t) *
                          CalcBSplineBasisFunc(knot, j + 1, p - 1, t) /
                          (knot[j + p + 1] - knot[j + 1]);
...
public:
  ...
  Curve(const void *data, int32_t size) {
    ...
    for (int i = 1; i < mControllPointCount; i++) {
      ...
      float len =
          Vector3D::Length(Vector3D((float)p1.X, (float)p1.Y, (float)p1.Z) -
                           Vector3D((float)p0.X, (float)p0.Y, (float)p0.Z));
      mLength += len;
    }
  ...
  Vector3D CalcuratePoint(float t, float magnification) {
    if (t == 0.0f && mControllPoint.size() > 0) {
      return {static_cast<float>(mControllPoint[0].X * magnification),
    ...
    std::vector<double> bs(
        mControllPointCount); // B-Spline 基底関数の計算結果(重み値を積算)
    ...
    for (int j = 0; j < mControllPointCount; ++j) {
      bs[j] =
          mControllPoint[j].W * CalcBSplineBasisFunc(knot, j, p, t * (t_rate));
    ...
    for (int j = 0; j < mControllPointCount; ++j) {
      Vector3D d;
      d.X = (float)mControllPoint[j].X * magnification * (float)bs[j] /
            (float)wSum;
      d.Y = (float)mControllPoint[j].Y * magnification * (float)bs[j] /
            (float)wSum;
      d.Z = (float)mControllPoint[j].Z * magnification * (float)bs[j] /
            (float)wSum;
...
public:
  ...
  */
  ...
  /**
          ...
          ::Effekseer::Effect::Create実行時に使用される。
  */
  virtual SoundDataRef Load(const char16_t *path) { return nullptr; }
  ...
  */
  virtual SoundDataRef Load(const void *data, int32_t size) { return nullptr; }
  ...
  /**
          ...
          ::Effekseer::Effectのインスタンスが破棄された時に使用される。
  */
  virtual void Unload(SoundDataRef data) { data.Reset(); }
};
