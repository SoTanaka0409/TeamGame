...
template <typename T, int N> class FixedSizeString {
  using Traits = std::char_traits<T>;

public:
  FixedSizeString() : size_(0) { data_[0] = 0; }
  ...
  FixedSizeString<T, N> &operator+=(const FixedSizeString<T, N> &c) {
    const auto new_size =
        (std::min)(this->size_ + c.size_, static_cast<size_t>(N));
...
public:
  template <typename T>
  static std::vector<std::basic_string<T>> Split(const std::basic_string<T> &s,
                                                 T delim) {
  ...
  template <typename T>
  static std::basic_string<T> Replace(std::basic_string<T> target,
                                      std::basic_string<T> from_,
                                      std::basic_string<T> to_) {
  ...
  Join(const std::vector<std::basic_string<T>> &elems,
...
private:
  template <typename T>
  static std::basic_string<T>
  Normalize(const std::vector<std::basic_string<T>> &paths) {
    std::vector<std::basic_string<T>> elems;
...
public:
  template <typename T> static bool IsSeparator(T s) {
  ...
  static std::basic_string<T> Normalize(const std::basic_string<T> &path) {
    ...
    auto paths = StringHelper::Split(
        StringHelper::Replace(path, StringHelper::To<T>("\\"),
                              StringHelper::To<T>("/")),
        static_cast<T>('/'));
  ...
  template <typename T>
  static std::basic_string<T> Relative(const std::basic_string<T> &targetPath,
                                       const std::basic_string<T> &basePath) {
    if (basePath.size() == 0 || targetPath.size() == 0) {
      return targetPath;
    }

    auto targetPaths = StringHelper::Split(
        StringHelper::Replace(targetPath, StringHelper::To<T>("\\"),
                              StringHelper::To<T>("/")),
        static_cast<T>('/'));
    auto basePaths = StringHelper::Split(
        StringHelper::Replace(basePath, StringHelper::To<T>("\\"),
                              StringHelper::To<T>("/")),
        static_cast<T>('/'));

    if (*(basePath.end() - 1) != static_cast<T>('/') &&
        *(basePath.end() - 1) != static_cast<T>('\\')) {
      basePaths.pop_back();
    }
  ...
  template <typename T>
  static std::basic_string<T> Absolute(const std::basic_string<T> &targetPath,
                                       const std::basic_string<T> &basePath) {
    ...
    std::copy(targetPaths.begin(), targetPaths.end(),
              std::back_inserter(basePaths));
  ...
  template <typename T>
  static std::basic_string<T>
  GetDirectoryName(const std::basic_string<T> &path) {
    return GetDirectoryName<std::basic_string<T>, T>(path);
  }

  template <typename T, int N>
  static FixedSizeString<T, N>
  GetDirectoryName(const FixedSizeString<T, N> &path) {
    return GetDirectoryName<FixedSizeString<T, N>, T>(path);
  }

  template <typename T>
  static std::basic_string<T>
  GetFilenameWithoutExt(const std::basic_string<T> &path) {
    return GetFilenameWithoutExt<std::basic_string<T>, T>(path);
  }

  template <typename T, int N>
  static FixedSizeString<T, N>
  GetFilenameWithoutExt(const FixedSizeString<T, N> &path) {
    return GetFilenameWithoutExt<FixedSizeString<T, N>, T>(path);
  }

  template <typename T>
  static std::basic_string<T> Combine(const std::basic_string<T> &path1,
                                      const std::basic_string<T> &path2) {
    return Combine<std::basic_string<T>, T>(path1, path2);
  }

  template <typename T, int N>
  static FixedSizeString<T, N> Combine(const FixedSizeString<T, N> &path1,
                                       const FixedSizeString<T, N> &path2) {
