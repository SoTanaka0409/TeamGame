...
public:
  ...
  */
  ...
  /**
  @if English
          @brief	Rotation matrix around an arbitrary axis
  (counter-clock-wise)
  ...
  */
  ...
  /**
  @if English
          @brief	Decompose matrix into scaling vector, rotation matrix
  ...
  */
  ...
  /**
  @if English
          @brief	Set scaling vector, rotation matrix and translation
  vector into matrix.
  ...
  */
  ...
  /**
          @brief
          \~English	check whether all values are not valid number (not nan,
     not inf)
          \~Japanese	全ての値が有効な数値かどうかを確認する。(nan でも inf
     でもない)
  */
  bool IsValid() const;
