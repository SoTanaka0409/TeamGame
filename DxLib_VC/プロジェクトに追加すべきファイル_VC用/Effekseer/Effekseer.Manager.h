...
/**
        @brief
        \~English Callback to query external collision. It returns true when a
   collision occurs and sets collisionPosition as the hit point and
   collisionNormal as the surface normal.
        \~Japanese
   外部との衝突を問い合わせるためのコールバック。衝突した場合はtrueを返し、collisionPositionに衝突点、collisionNormalに衝突面の法線を設定する。
*/
using CollisionCallback = std::function<bool(
    const Vector3D &startPosition, const Vector3D &endPosition,
    Vector3D &collisionPosition, Vector3D &collisionNormal)>;
  ...
  struct UpdateParameter {
    ...
    */
    ...
    /**
       ...
       call anything other than Draw or Compute)
            \~Japanese
       trueなら同期的に更新処理を行う。falseなら非同期的に更新処理を行う（次はDraw/Compute以外呼び出してはいけない）
    */
...
public:
  ...
  */
  ...
  /**
          @brief
          \~English Set a callback to determine collision points against
     external objects.
          \~Japanese
     外部オブジェクトへの衝突判定に使用するコールバックを設定する。
  */
  virtual void SetCollisionCallback(CollisionCallback callback) = 0;

  /**
          @brief
          \~English Get a callback to determine collision points against
     external objects.
          \~Japanese
     外部オブジェクトへの衝突判定に使用するコールバックを取得する。
  */
  ...
  */
  ...
  /**
          @brief
          \~English	Gets the GPU time (microseconds) taken to render the all
     effects.
          \~Japanese
     エフェクト全ての描画処理にかかるGPU時間(マイクロ秒)を取得する。
  */
  virtual int32_t GetGpuTime() const = 0;

  /**
          @brief
          \~English	Gets the GPU time (microseconds) taken to render the
     effect.
          \~Japanese
     エフェクトの描画処理にかかるGPU時間(マイクロ秒)を取得する。
  */
