...
private:
  template <typename T> struct LoadCounted {
    T resource;
    int32_t loadCount;
  };

  template <typename PARAMETER, typename T> struct GenerateCounted {
    PARAMETER param;
    T resource;
    int32_t loadCount;
  };
...
public:
  template <typename LOADER, typename RESOURCE> class CachedResources {
    bool isCacheEnabled_ = true;
    LOADER loader_;
    CustomUnorderedMap<StringView<char16_t>, LoadCounted<RESOURCE>,
                       StringView<char16_t>::Hash>
        cached_;

  public:
    bool GetIsCacheEnabled() const { return isCacheEnabled_; }

    void SetIsCacheEnabled(bool value) { isCacheEnabled_ = value; }

    LOADER GetLoader() const { return loader_; }

    void SetLoader(LOADER value) { loader_ = value; }
  ...
  public:
    ...
    template <typename... Arg> RESOURCE Load(const PARAMETER &parameter) {
      if (loader_ != nullptr) {
        if (isCacheEnabled_) {
          ...
          if (resource != nullptr) {
            cached_.emplace(parameter, GenerateCounted<PARAMETER, RESOURCE>{
                                           parameter, resource, 1});
            return resource;
          }
        } else {
    ...
    void Unload(const RESOURCE &resource) {
      if (loader_ != nullptr && resource != nullptr) {
        auto it = std::find_if(
            ...
            [&](const std::pair<PARAMETER, GenerateCounted<PARAMETER, RESOURCE>>
                    &v) { return v.second.resource == resource; });
        if (it != cached_.end()) {
          if (--it->second.loadCount <= 0) {
            cached_.erase(it);
  ...
  ModelLoaderRef GetModelLoader() const { return CachedModels.GetLoader(); }

  void SetModelLoader(ModelLoaderRef loader) { CachedModels.SetLoader(loader); }

  SoundLoaderRef GetSoundLoader() const { return CachedSounds.GetLoader(); }

  void SetSoundLoader(SoundLoaderRef loader) { CachedSounds.SetLoader(loader); }
  ...
  CurveLoaderRef GetCurveLoader() const { return CachedCurves.GetLoader(); }

  void SetCurveLoader(CurveLoaderRef loader) { CachedCurves.SetLoader(loader); }
  ...
  CachedParameterResources<ProceduralModelGeneratorRef,
                           ProceduralModelParameter, ModelRef>
      CachedProceduralModels;
};
