...
public:
  ...
  Vector3D CalcuratePoint(float t, float magnification) {
    if (t == 0.0f && controlPoints_.size() > 0) {
      return {static_cast<float>(controlPoints_[0].X * magnification),
    ...
    std::vector<double> bs(
        controlPointCount_); // B-Spline 基底関数の計算結果(重み値を積算)
    ...
    for (int j = 0; j < controlPointCount_; ++j) {
      bs[j] =
          controlPoints_[j].W * CalcBSplineBasisFunc(knot, j, p, t * (t_rate));
    ...
    for (int j = 0; j < controlPointCount_; ++j) {
      Vector3D d;
      d.X = (float)controlPoints_[j].X * magnification * (float)bs[j] /
            (float)wSum;
      d.Y = (float)controlPoints_[j].Y * magnification * (float)bs[j] /
            (float)wSum;
      d.Z = (float)controlPoints_[j].Z * magnification * (float)bs[j] /
            (float)wSum;
  ...
  //  Getter
  //
  int GetControllPointCount() { return controlPointCount_; }
  dVector4 GetControllPoint(int index) { return controlPoints_[index]; }

  int GetKnotCount() { return knotCount_; }
  double GetKnotValue(int index) { return knotValues_[index]; }
