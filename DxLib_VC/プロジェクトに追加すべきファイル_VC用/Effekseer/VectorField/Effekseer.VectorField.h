...
public:
  ...
  std::array<float, 3> GetData(const std::array<int32_t, 3> &position,
                               VectorFieldSamplingMode sampling_mode) const;

  void SetData(const std::array<float, 3> &value,
               const std::array<int32_t, 3> &position);

  std::array<float, 3> GetData(const std::array<float, 3> &position,
                               VectorFieldSamplingMode sampling_mode) const;
};
