...
protected:
  ...
  void RenderSplines(const efkRibbonNodeParam &parameter,
    ...
    if (parameter.SplineDivision > 1) {
      ...
      for (size_t loop = 0; loop < instances.size(); loop++) {
        ...
        if (parameter.ViewpointDependent) {
          ...
          F = ::Effekseer::SIMD::Vec3f(-renderer_->GetCameraFrontDirection())
                  .GetNormal();
          R = ::Effekseer::SIMD::Vec3f::Cross(U, F).GetNormal();
          F = ::Effekseer::SIMD::Vec3f::Cross(R, U).GetNormal();

          ::Effekseer::SIMD::Mat43f mat_rot(
              -R.GetX(), -R.GetY(), -R.GetZ(), U.GetX(), U.GetY(), U.GetZ(),
              F.GetX(), F.GetY(), F.GetZ(), t.GetX(), t.GetY(), t.GetZ());
    ...
    for (size_t loop = 0; loop < instances.size(); loop++) {
      ...
      for (auto sploop = 0; sploop < parameter.SplineDivision; sploop++) {
        ...
        if (parameter.SplineDivision > 1) {
          verteies[0].Pos = ToStruct(spline_left.GetValue(
              param.InstanceIndex + sploop / (float)parameter.SplineDivision));
          verteies[5].Pos = ToStruct(spline_right.GetValue(
              param.InstanceIndex + sploop / (float)parameter.SplineDivision));

          verteies[0].SetColor(Effekseer::Color::Lerp(param.Colors[0],
                                                      param.Colors[2],
                                                      percent_instance),
                               FLIP_RGB);
          verteies[5].SetColor(Effekseer::Color::Lerp(param.Colors[1],
                                                      param.Colors[3],
                                                      percent_instance),
                               FLIP_RGB);
          ...
          verteies[1].Col =
              Effekseer::Color::Lerp(verteies[0].Col, verteies[5].Col, 0.5f);
        ...
        } else {
          {
            ...
            verteies[0].SetFlipbookIndexAndNextRate(
                param.FlipbookIndexAndNextRate);
...

          {
            ...
            verteies[5].SetFlipbookIndexAndNextRate(
                param.FlipbookIndexAndNextRate);
          ...
          verteies[1].Col =
              Effekseer::Color::Lerp(verteies[0].Col, verteies[5].Col, 0.5f);
          verteies[1].SetFlipbookIndexAndNextRate(
              param.FlipbookIndexAndNextRate);
        ...
        if (parameter.ViewpointDependent) {
          ...
          if (parameter.SplineDivision > 1) {
            ...
            for (int i : {0, 1, 4, 5}) {
              verteies[i].Pos = ToStruct(::Effekseer::SIMD::Vec3f::Transform(
                  verteies[i].Pos, mat_rot));
            }
          }
        } else {
          if (parameter.SplineDivision > 1) {
            ...
            for (int i : {0, 1, 4, 5}) {
              verteies[i].Pos = ToStruct(
                  ::Effekseer::SIMD::Vec3f::Transform(verteies[i].Pos, mat));
            }
          }
        }

        for (int idx : {0, 1, 4, 5}) {
          verteies[idx].SetParticleTime(param.ParticleTimes[0],
                                        param.ParticleTimes[1]);
        }
    ...
    if (VertexNormalRequired<VERTEX>()) {
      ...
      for (size_t i = 0;
        ...
        Effekseer::SIMD::Vec3f normal =
            Effekseer::SIMD::Vec3f::Cross(axis, tangent);
    ...
    TrailRendererUtils::AssignUVs<VERTEX, efkRibbonInstanceParam, 0>(
        *parameter.TextureUVTypeParameterPtr, instances, verteies,
        parameter.SplineDivision, global_scale);

    if (VertexUV2Required<VERTEX>()) {
      TrailRendererUtils::AssignUVs<VERTEX, efkRibbonInstanceParam, 1>(
          *parameter.TextureUVTypeParameterPtr, instances, verteies,
          parameter.SplineDivision, global_scale);
    }

    TrailRendererUtils::AssignUVs<VERTEX, efkRibbonInstanceParam, 2>(
        *parameter.TextureUVTypeParameterPtr, instances, verteies,
        parameter.SplineDivision, global_scale);
    TrailRendererUtils::AssignUVs<VERTEX, efkRibbonInstanceParam, 3>(
        *parameter.TextureUVTypeParameterPtr, instances, verteies,
        parameter.SplineDivision, global_scale);
    TrailRendererUtils::AssignUVs<VERTEX, efkRibbonInstanceParam, 4>(
        *parameter.TextureUVTypeParameterPtr, instances, verteies,
        parameter.SplineDivision, global_scale);
    TrailRendererUtils::AssignUVs<VERTEX, efkRibbonInstanceParam, 5>(
        *parameter.TextureUVTypeParameterPtr, instances, verteies,
        parameter.SplineDivision, global_scale);
    TrailRendererUtils::AssignUVs<VERTEX, efkRibbonInstanceParam, 6>(
        *parameter.TextureUVTypeParameterPtr, instances, verteies,
        parameter.SplineDivision, global_scale);
...
protected:
  ...
  void Rendering_Internal(const efkRibbonNodeParam &parameter,
    ...
    bool isLast = instanceParameter.InstanceIndex ==
                  (instanceParameter.InstanceCount - 1);
...
public:
  void BeginRenderingGroup(const efkRibbonNodeParam &param, int32_t count,
  ...
  void Rendering(const efkRibbonNodeParam &parameter,
                 const efkRibbonInstanceParam &instanceParameter,
                 void *userData) override {
    Rendering_(parameter, instanceParameter, renderer_->GetCameraMatrix());
  }

  void EndRendering(const efkRibbonNodeParam &parameter,
                    void *userData) override {
