...
inline Effekseer::Color PackVector3DF(const Effekseer::SIMD::Vec3f &v) {
  Effekseer::Color ret;
  ret.R = static_cast<uint8_t>(Effekseer::Clamp(
      ((v.GetX() + 1.0f) / 2.0f + 0.5f / 255.0f) * 255.0f, 255, 0));
  ret.G = static_cast<uint8_t>(Effekseer::Clamp(
      ((v.GetY() + 1.0f) / 2.0f + 0.5f / 255.0f) * 255.0f, 255, 0));
  ret.B = static_cast<uint8_t>(Effekseer::Clamp(
      ((v.GetZ() + 1.0f) / 2.0f + 0.5f / 255.0f) * 255.0f, 255, 0));
  ret.A = 255;
  return ret;
}
...
template <typename U> class ContainAdvancedData {
public:
  using Value = int;
};

template <> class ContainAdvancedData<SimpleVertex> {
public:
  using Value = float;
};

template <> class ContainAdvancedData<LightingVertex> {
public:
  using Value = float;
};

template <> class ContainAdvancedData<DynamicVertex> {
public:
  using Value = float;
};

template <typename U>
using enable_if_contain_advanced_t = typename std::enable_if<
    std::is_same<typename ContainAdvancedData<U>::Value, int>::value,
    std::nullptr_t>::type;

template <typename U>
using enable_ifnot_contain_advanced_t = typename std::enable_if<
    std::is_same<typename ContainAdvancedData<U>::Value, float>::value,
    std::nullptr_t>::type;
...
template <typename T> inline bool VertexNormalRequired() { return false; }

template <> inline bool VertexNormalRequired<DynamicVertex>() { return true; }

template <> inline bool VertexNormalRequired<LightingVertex>() { return true; }

template <> inline bool VertexNormalRequired<AdvancedLightingVertex>() {
  return true;
}

template <typename T> inline bool VertexUV2Required() { return false; }

template <> inline bool VertexUV2Required<DynamicVertex>() { return true; }
...
template <typename T> struct StrideView {
  ...
  StrideView(void *pointer, int32_t stride, int32_t elementCount)
      : stride_(stride), pointer_(reinterpret_cast<uint8_t *>(pointer)),
        pointerOrigin_(reinterpret_cast<uint8_t *>(pointer))
#ifndef NDEBUG
        ,
        offset_(0), elementCount_(elementCount)
#endif
  {
  }
...
std::array<std::array<float, 4>, 13>
ToUniform(const Effekseer::Gradient &gradient);

void CalcBillboard(::Effekseer::BillboardType billboardType,
                   Effekseer::SIMD::Mat43f &dst, ::Effekseer::SIMD::Vec3f &s,
                   ::Effekseer::SIMD::Vec3f &R, ::Effekseer::SIMD::Vec3f &F,
...
void ApplyDepthParameters(
    ::Effekseer::SIMD::Mat43f &mat, const ::Effekseer::SIMD::Vec3f &cameraFront,
    const ::Effekseer::SIMD::Vec3f &cameraPos,
    ::Effekseer::NodeRendererDepthParameter *depthParameter, bool isRightHand);
...
void ApplyDepthParameters(
    ::Effekseer::SIMD::Mat43f &mat, ::Effekseer::SIMD::Vec3f &translationValues,
    ::Effekseer::SIMD::Vec3f &scaleValues,
...
void ApplyDepthParameters(
    ::Effekseer::SIMD::Mat44f &mat, const ::Effekseer::SIMD::Vec3f &cameraFront,
    const ::Effekseer::SIMD::Vec3f &cameraPos,
    ::Effekseer::NodeRendererDepthParameter *depthParameter, bool isRightHand);

void ApplyViewOffset(::Effekseer::SIMD::Mat43f &mat,
                     const ::Effekseer::SIMD::Mat44f &camera, float distance);
...
inline void TransformVertexes(Vertex &vertexes, int32_t count,
                              const ::Effekseer::SIMD::Mat43f &mat) {
  using namespace Effekseer::SIMD;
...
struct MaterialShaderParameterGenerator {
  ...
  MaterialShaderParameterGenerator(
      const ::Effekseer::MaterialFile &materialFile, bool isModel,
      int32_t stage, int32_t instanceCount) {
    if (isModel) {
      ...
      VertexSize = sizeof(EffekseerRenderer::DynamicVertex) +
                   sizeof(float) * (materialFile.GetCustomData1Count() +
                                    materialFile.GetCustomData2Count());
    }
...
struct ShaderParameterCollector {
  ...
  std::array<::Effekseer::Backend::TextureRef, Effekseer::TextureSlotMax>
      Textures;
  ...
  bool operator!=(const ShaderParameterCollector &state) const {
    ...
    if (IsBackgroundRequiredOnFirstPass !=
        state.IsBackgroundRequiredOnFirstPass)
      return true;
  ...
  void Collect(Renderer *renderer, Effekseer::Effect *effect,
               Effekseer::NodeRendererBasicParameter *param, bool edgeFalloff,
    ...
    auto isMaterial =
        param->MaterialType == ::Effekseer::RendererMaterialType::File &&
        param->MaterialRenderDataPtr != nullptr &&
        renderer->GetRenderMode() == Effekseer::RenderMode::Normal;

    if (isMaterial) {
      MaterialDataPtr =
          effect->GetMaterial(param->MaterialRenderDataPtr->MaterialIndex);
      ...
      // Validate parameters
      if (isMaterial &&
          (MaterialDataPtr->TextureCount !=
               param->MaterialRenderDataPtr->MaterialTextures.size() ||
           MaterialDataPtr->UniformCount !=
               param->MaterialRenderDataPtr->MaterialUniforms.size())) {
        isMaterial = false;
      }
    }
    ...
    const auto whiteMode =
        renderer->GetRenderMode() == Effekseer::RenderMode::Wireframe ||
        renderer->GetExternalShaderSettings() != nullptr;
    if (whiteMode) {
      ShaderType = RendererShaderType::Unlit;
    } else if (param->MaterialType == ::Effekseer::RendererMaterialType::File &&
               isMaterial) {
      MaterialRenderDataPtr = param->MaterialRenderDataPtr;
      if (MaterialRenderDataPtr != nullptr) {
        MaterialDataPtr =
            effect->GetMaterial(MaterialRenderDataPtr->MaterialIndex);
        if (MaterialDataPtr != nullptr) {
          ShaderType = RendererShaderType::Material;
          IsBackgroundRequiredOnFirstPass =
              MaterialDataPtr->IsRefractionRequired;
    ...
    } else if (param->MaterialType ==
                   ::Effekseer::RendererMaterialType::Lighting &&
               isAdvanced) {
      ShaderType = RendererShaderType::AdvancedLit;
    } else if (param->MaterialType ==
                   ::Effekseer::RendererMaterialType::BackDistortion &&
               isAdvanced) {
      ShaderType = RendererShaderType::AdvancedBackDistortion;
      IsBackgroundRequiredOnFirstPass = true;
    } else if (param->MaterialType ==
                   ::Effekseer::RendererMaterialType::Default &&
               isAdvanced) {
    ...
    if (whiteMode) {
      TextureCount = 1;
      Textures[0] = renderer->GetImpl()->GetProxyTexture(
      ...
      TextureCount = static_cast<int32_t>(
          Effekseer::Min(MaterialRenderDataPtr->MaterialTextures.size(),
                         ::Effekseer::UserTextureSlotMax));
      for (int32_t i = 0; i < TextureCount; i++) {
        if (MaterialRenderDataPtr->MaterialTextures[i].Type == 1) {
          if (MaterialRenderDataPtr->MaterialTextures[i].Index >= 0) {
            auto resource = effect->GetNormalImage(
                MaterialRenderDataPtr->MaterialTextures[i].Index);
            Textures[i] =
                (resource != nullptr) ? resource->GetBackend() : nullptr;
          } else {
            Textures[i] = nullptr;
          }
        } else {
          if (MaterialRenderDataPtr->MaterialTextures[i].Index >= 0) {
            auto resource = effect->GetColorImage(
                MaterialRenderDataPtr->MaterialTextures[i].Index);
            Textures[i] =
                (resource != nullptr) ? resource->GetBackend() : nullptr;
          } else {
      ...
      if (TexturePtr == nullptr && renderer != nullptr) {
        TexturePtr = renderer->GetImpl()->GetProxyTexture(
            EffekseerRenderer::ProxyTextureType::White);
      }
      ...
      if (param->MaterialType != ::Effekseer::RendererMaterialType::Default) {
        if (param->MaterialType ==
          ...
          if (NormalTexturePtr == nullptr && renderer != nullptr) {
            NormalTexturePtr = renderer->GetImpl()->GetProxyTexture(
                EffekseerRenderer::ProxyTextureType::Normal);
          }
      ...
      if (isAdvanced) {
        ...
        if (AlphaTexturePtr == nullptr && renderer != nullptr) {
          AlphaTexturePtr = renderer->GetImpl()->GetProxyTexture(
              EffekseerRenderer::ProxyTextureType::White);
        }
        ...
        if (UVDistortionTexturePtr == nullptr && renderer != nullptr) {
          UVDistortionTexturePtr = renderer->GetImpl()->GetProxyTexture(
              EffekseerRenderer::ProxyTextureType::Normal);
        }
        ...
        if (BlendTexturePtr == nullptr && renderer != nullptr) {
          BlendTexturePtr = renderer->GetImpl()->GetProxyTexture(
              EffekseerRenderer::ProxyTextureType::White);
        }
        ...
        if (BlendAlphaTexturePtr == nullptr && renderer != nullptr) {
          BlendAlphaTexturePtr = renderer->GetImpl()->GetProxyTexture(
              EffekseerRenderer::ProxyTextureType::White);
        }
        ...
        if (BlendUVDistortionTexturePtr == nullptr && renderer != nullptr) {
          BlendUVDistortionTexturePtr = renderer->GetImpl()->GetProxyTexture(
              EffekseerRenderer::ProxyTextureType::Normal);
        }
...
struct SoftParticleParameter {
  ...
  void SetParam(float distanceFar, float distanceNear, float distanceNearOffset,
                float magnification, float rescale1, float rescale2, float v33,
                float v34, float v43, float v44) {
    ...
    softParticleParams[3] = distanceFar != 0.0f || distanceNear != 0.0f ||
                                    distanceNearOffset != 0.0f
                                ? 1.0f
                                : 0.0f;
...
struct PixelConstantBuffer {
  ...
  void SetModelFlipbookParameter(float enableInterpolation,
                                 float interpolationType) {
    FlipbookParam.EnableInterpolation = enableInterpolation;
    FlipbookParam.InterpolationType = interpolationType;
  }

  void SetModelUVDistortionParameter(float intensity, float blendIntensity,
                                     const std::array<float, 2> &uvInversed) {
  ...
  void SetFalloffParameter(float enable, float colorBlendType, float pow,
                           const std::array<float, 4> &beginColor,
  ...
  void SetEdgeParameter(const std::array<float, 4> &edgeColor, float threshold,
                        float colorScaling) {
    for (size_t i = 0; i < 4; i++) {
...
void CalculateAlignedTextureInformation(
    Effekseer::Backend::TextureFormatType format,
    const std::array<int, 2> &size, int32_t &sizePerWidth, int32_t &height);

//! only support OpenGL, DirectX11
Effekseer::Backend::VertexLayoutRef
GetVertexLayout(Effekseer::Backend::GraphicsDeviceRef graphicsDevice,
                RendererShaderType type);

Effekseer::Backend::VertexLayoutRef GetModelRendererVertexLayout(
    Effekseer::Backend::GraphicsDeviceRef graphicsDevice);

Effekseer::Backend::VertexLayoutRef GetMaterialSimpleVertexLayout(
    Effekseer::Backend::GraphicsDeviceRef graphicsDevice);

Effekseer::Backend::VertexLayoutRef GetMaterialSpriteVertexLayout(
    Effekseer::Backend::GraphicsDeviceRef graphicsDevice, int32_t customData1,
    int32_t customData2);

Effekseer::Backend::VertexLayoutRef GetMaterialModelVertexLayout(
    Effekseer::Backend::GraphicsDeviceRef graphicsDevice);
...
inline RendererStateFlipbook
ToState(const Effekseer::NodeRendererFlipbookParameter &param) {
...
template <typename T>
bool GenerateIndexDataStride(
    Effekseer::Backend::GraphicsDeviceRef graphicsDevice,
    int32_t squareMaxCount, Effekseer::Backend::IndexBufferRef &indexBuffer,
    Effekseer::Backend::IndexBufferRef &indexBufferForWireframe) {
  auto stride = sizeof(T) == 2
                    ? Effekseer::Backend::IndexBufferStrideType::Stride2
                    : Effekseer::Backend::IndexBufferStrideType::Stride4;

  {
    ...
    indexBuffer = graphicsDevice->CreateIndexBuffer(squareMaxCount * 6,
                                                    buffer.data(), stride);
...

  {
    ...
    indexBufferForWireframe = graphicsDevice->CreateIndexBuffer(
        squareMaxCount * 8, buffer.data(), stride);
