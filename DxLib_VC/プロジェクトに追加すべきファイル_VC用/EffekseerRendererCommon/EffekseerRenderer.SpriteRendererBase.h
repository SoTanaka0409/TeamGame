...
public:
  SpriteRendererBase(RENDERER *renderer)
      : renderer_(renderer), spriteCount_(0), ringBufferData_(nullptr) {
    // reserve buffers
...
protected:
  ...
  void BeginRendering_(RENDERER *renderer, int32_t count,
                       const efkSpriteNodeParam &param, void *userData) {
    ...
    renderer->GetStandardRenderer()->BeginRenderingAndRenderingIfRequired(
        state, vertexCount_, stride_, (void *&)ringBufferData_);
    spriteCount_ = 0;
  ...
  void Rendering_Internal(const efkSpriteNodeParam &parameter,
    ...
    SetVertexAlphaUV(
        verteies[0],
        instanceParameter.AlphaUV.Y + instanceParameter.AlphaUV.Height, 1);

    SetVertexAlphaUV(
        verteies[1],
        instanceParameter.AlphaUV.X + instanceParameter.AlphaUV.Width, 0);
    SetVertexAlphaUV(
        verteies[1],
        instanceParameter.AlphaUV.Y + instanceParameter.AlphaUV.Height, 1);
    ...
    SetVertexUVDistortionUV(verteies[0],
                            instanceParameter.UVDistortionUV.Y +
                                instanceParameter.UVDistortionUV.Height,
                            1);

    SetVertexUVDistortionUV(verteies[1],
                            instanceParameter.UVDistortionUV.X +
                                instanceParameter.UVDistortionUV.Width,
                            0);
    SetVertexUVDistortionUV(verteies[1],
                            instanceParameter.UVDistortionUV.Y +
                                instanceParameter.UVDistortionUV.Height,
    ...
    SetVertexUVDistortionUV(verteies[3],
                            instanceParameter.UVDistortionUV.X +
                                instanceParameter.UVDistortionUV.Width,
                            0);
    ...
    SetVertexBlendUV(
        verteies[0],
        instanceParameter.BlendUV.Y + instanceParameter.BlendUV.Height, 1);

    SetVertexBlendUV(
        verteies[1],
        instanceParameter.BlendUV.X + instanceParameter.BlendUV.Width, 0);
    SetVertexBlendUV(
        verteies[1],
        instanceParameter.BlendUV.Y + instanceParameter.BlendUV.Height, 1);
    ...
    SetVertexBlendAlphaUV(verteies[0],
                          instanceParameter.BlendAlphaUV.Y +
                              instanceParameter.BlendAlphaUV.Height,
                          1);

    SetVertexBlendAlphaUV(verteies[1],
                          instanceParameter.BlendAlphaUV.X +
                              instanceParameter.BlendAlphaUV.Width,
                          0);
    SetVertexBlendAlphaUV(verteies[1],
                          instanceParameter.BlendAlphaUV.Y +
                              instanceParameter.BlendAlphaUV.Height,
    ...
    SetVertexBlendAlphaUV(verteies[3],
                          instanceParameter.BlendAlphaUV.X +
                              instanceParameter.BlendAlphaUV.Width,
                          0);
    SetVertexBlendAlphaUV(verteies[3], instanceParameter.BlendAlphaUV.Y, 1);

    SetVertexBlendUVDistortionUV(verteies[0],
                                 instanceParameter.BlendUVDistortionUV.X, 0);
    SetVertexBlendUVDistortionUV(
        verteies[0],
        instanceParameter.BlendUVDistortionUV.Y +
            instanceParameter.BlendUVDistortionUV.Height,
        1);

    SetVertexBlendUVDistortionUV(
        verteies[1],
        instanceParameter.BlendUVDistortionUV.X +
            instanceParameter.BlendUVDistortionUV.Width,
        0);
    SetVertexBlendUVDistortionUV(
        verteies[1],
        instanceParameter.BlendUVDistortionUV.Y +
    ...
    SetVertexBlendUVDistortionUV(verteies[2],
                                 instanceParameter.BlendUVDistortionUV.X, 0);
    SetVertexBlendUVDistortionUV(verteies[2],
                                 instanceParameter.BlendUVDistortionUV.Y, 1);
    ...
    SetVertexBlendUVDistortionUV(
        ...
        instanceParameter.BlendUVDistortionUV.X +
            instanceParameter.BlendUVDistortionUV.Width,
        0);
    SetVertexBlendUVDistortionUV(verteies[3],
                                 instanceParameter.BlendUVDistortionUV.Y, 1);
    ...
    if (parameter.Billboard != ::Effekseer::BillboardType::Fixed) {
      ...
      if (parameter.EnableViewOffset == true) {
        ...
        CalcBillboard(parameter.Billboard, mat_rot, s, R, F, instMat,
                      renderer_->GetCameraFrontDirection(),
                      instanceParameter.Direction);
      } else {
        CalcBillboard(parameter.Billboard, mat_rot, s, R, F,
      ...
      ApplyDepthParameters(mat_rot, renderer_->GetCameraFrontDirection(),
...
public:
  void BeginRendering(const efkSpriteNodeParam &parameter, int32_t count,
                      void *userData) override {
    BeginRendering_(renderer_, count, parameter, userData);
  }

  void Rendering(const efkSpriteNodeParam &parameter,
                 const efkSpriteInstanceParam &instanceParameter,
                 void *userData) override {
    if (ringBufferData_ == nullptr)
      return;
  ...
  void EndRendering(const efkSpriteNodeParam &parameter,
                    void *userData) override {
    if (ringBufferData_ == nullptr)
      return;
