...
public:
  ...
  static void AssignUV(StrideView<VERTEX> &v, float uvX1, float uvX2,
                       float uvX3, float uvY1, float uvY2) {
    if (TARGET == 0) {
      v[0].UV[0] = uvX1;
      v[0].UV[1] = uvY1;
  ...
  static void
  AssignUVs(const Effekseer::NodeRendererTextureUVTypeParameter &uvParam,
            Effekseer::CustomAlignedVector<INSTANCE> &instances,
            StrideView<VERTEX> verteies, int spline_division,
            float globalScale) {
    float uvx = 0.0f;
    float uvw = 1.0f;
    float uvy = 0.0f;
    ...
    if (uvParam.Type == ::Effekseer::TextureUVType::Strech) {
      ...
      for (size_t loop = 0; loop < instances.size() - 1; loop++) {
        ...
        for (int32_t sploop = 0; sploop < spline_division; sploop++) {
          float percent1 =
              (float)(param.InstanceIndex * spline_division + sploop) /
              (float)((param.InstanceCount - 1) * spline_division);

          float percent2 =
              (float)(param.InstanceIndex * spline_division + sploop + 1) /
              (float)((param.InstanceCount - 1) * spline_division);
        ...
        if (loop < uvParam.TileEdgeTail) {
          ...
          for (int32_t sploop = 0; sploop < spline_division; sploop++) {
            const int headOffset =
                param.InstanceCount - 1 - uvParam.TileEdgeHead;
            float percent1 =
                (float)((param.InstanceIndex - headOffset) * spline_division +
                        sploop) /
      ...
      for (size_t loop = 0; loop < instances.size() - 1; loop++) {
        const float length = (instances[loop + 1].SRTMatrix43.GetTranslation() -
                              instances[loop].SRTMatrix43.GetTranslation())
                                 .GetLength();
        lengthAll += length;
      }
