...
protected:
  ...
  void RenderSplines(const efkTrackNodeParam &parameter,
    ...
    if (parameter.SmoothingType == Effekseer::TrailSmoothingType::On) {
      ...
      for (size_t i = 0; i < instances.size(); i++) {
        ...
        if (i == 0) {
          axis = (instances[i + 1].SRTMatrix43.GetTranslation() -
                  instances[i].SRTMatrix43.GetTranslation());
        } else if (i == instances.size() - 1) {
          axis = (instances[i].SRTMatrix43.GetTranslation() -
                  instances[i - 1].SRTMatrix43.GetTranslation());
        } else {
          axis = (instances[i + 1].SRTMatrix43.GetTranslation() -
                  instances[i - 1].SRTMatrix43.GetTranslation());
        }
        ...
        auto qq = (q.s * q.s);
        auto len =
            sqrtf(qq.GetX() + qq.GetY() + qq.GetZ() + qq.GetW()) + 0.00001f;
      ...
      for (size_t i = 1; i < instances.size() - 1; i++) {
        const auto q1 = Effekseer::SIMD::Quaternionf::Slerp(
            rotations_temp_[i - 1], rotations_temp_[i + 1], 0.5f);
        rotations_[i] = Effekseer::SIMD::Quaternionf::Slerp(
            q1, rotations_temp_[i], 2.0f / 3.0f);
      }
    }
    ...
    if (parameter.SplineDivision > 1) {
      ...
      for (size_t loop = 0; loop < instances.size(); loop++) {
        ...
        ApplyDepthParameters(mat, renderer_->GetCameraFrontDirection(),
    ...
    for (size_t loop = 0; loop < instances.size(); loop++) {
      ...
      for (int32_t sploop = 0; sploop < parameter.SplineDivision; sploop++) {
        ...
        ApplyDepthParameters(r, t, s, renderer_->GetCameraFrontDirection(),
                             renderer_->GetCameraPosition(),
                             parameter.DepthParameterPtr,
                             parameter.IsRightHand);
        ...
        float percent =
            (float)(param.InstanceIndex * parameter.SplineDivision + sploop) /
        ...
        if (param.InstanceIndex < param.InstanceCount / 2) {
          ...
          leftColor.R = (uint8_t)Effekseer::Clamp(
              param.ColorLeft.R +
                  (param.ColorLeftMiddle.R - param.ColorLeft.R) * l,
              255, 0);
          leftColor.G = (uint8_t)Effekseer::Clamp(
              param.ColorLeft.G +
                  (param.ColorLeftMiddle.G - param.ColorLeft.G) * l,
              255, 0);
          leftColor.B = (uint8_t)Effekseer::Clamp(
              param.ColorLeft.B +
                  (param.ColorLeftMiddle.B - param.ColorLeft.B) * l,
              255, 0);
          leftColor.A = (uint8_t)Effekseer::Clamp(
              param.ColorLeft.A +
                  (param.ColorLeftMiddle.A - param.ColorLeft.A) * l,
              255, 0);

          centerColor.R = (uint8_t)Effekseer::Clamp(
              param.ColorCenter.R +
                  (param.ColorCenterMiddle.R - param.ColorCenter.R) * l,
              255, 0);
          centerColor.G = (uint8_t)Effekseer::Clamp(
              param.ColorCenter.G +
                  (param.ColorCenterMiddle.G - param.ColorCenter.G) * l,
              255, 0);
          centerColor.B = (uint8_t)Effekseer::Clamp(
              param.ColorCenter.B +
                  (param.ColorCenterMiddle.B - param.ColorCenter.B) * l,
              255, 0);
          centerColor.A = (uint8_t)Effekseer::Clamp(
              param.ColorCenter.A +
                  (param.ColorCenterMiddle.A - param.ColorCenter.A) * l,
              255, 0);

          rightColor.R = (uint8_t)Effekseer::Clamp(
              param.ColorRight.R +
                  (param.ColorRightMiddle.R - param.ColorRight.R) * l,
              255, 0);
          rightColor.G = (uint8_t)Effekseer::Clamp(
              param.ColorRight.G +
                  (param.ColorRightMiddle.G - param.ColorRight.G) * l,
              255, 0);
          rightColor.B = (uint8_t)Effekseer::Clamp(
              param.ColorRight.B +
                  (param.ColorRightMiddle.B - param.ColorRight.B) * l,
              255, 0);
          rightColor.A = (uint8_t)Effekseer::Clamp(
              param.ColorRight.A +
                  (param.ColorRightMiddle.A - param.ColorRight.A) * l,
              255, 0);
        } else {
          float l = percent;
          l = 1.0f - (l * 2.0f - 1.0f);
        ...
        if (parameter.SplineDivision > 1) {
          v[1].Pos = ToStruct(spline.GetValue(
              param.InstanceIndex + sploop / (float)parameter.SplineDivision));
        } else {
          v[1].Pos = ToStruct(t);
        }
    ...
    // transform all vertecies
    {
      ...
      for (size_t i = 0;
           i < (instances.size() - 1) * parameter.SplineDivision + 1; i++) {
        bool isFirst_ = (i == 0);
        bool isLast_ =
            (i == ((instances.size() - 1) * parameter.SplineDivision));
        Effekseer::SIMD::Vec3f axis;
        ...
        if (parameter.SmoothingType == Effekseer::TrailSmoothingType::On) {
          ...
          if (isLast_) {
            ...
            rotq = Effekseer::SIMD::Quaternionf::Slerp(
                q0, q1, splInd / static_cast<float>(parameter.SplineDivision));
          }

          const auto rdir =
              Effekseer::SIMD::Quaternionf::Transform({-1, 0, 0}, rotq);
          ...
          F =
          ::Effekseer::SIMD::Vec3f(renderer_->GetCameraFrontDirection()).Normalize();
          ...
          vl.Pos = ToStruct(::Effekseer::SIMD::Vec3f::Transform(vl.Pos,
          mat_rot)); vm.Pos =
          ToStruct(::Effekseer::SIMD::Vec3f::Transform(vm.Pos, mat_rot)); vr.Pos
          = ToStruct(::Effekseer::SIMD::Vec3f::Transform(vr.Pos,mat_rot));
          */
        ...
        if (VertexNormalRequired<VERTEX>()) {
          ::Effekseer::SIMD::Vec3f tangent =
              SafeNormalize(Effekseer::SIMD::Vec3f(vl.Pos - vr.Pos));
          Effekseer::SIMD::Vec3f normal =
              SafeNormalize(Effekseer::SIMD::Vec3f::Cross(tangent, axis));
    ...
    TrailRendererUtils::AssignUVs<VERTEX, efkTrackInstanceParam, 0>(
        *parameter.TextureUVTypeParameterPtr, instances, verteies,
        parameter.SplineDivision, global_scale);

    if (VertexUV2Required<VERTEX>()) {
      TrailRendererUtils::AssignUVs<VERTEX, efkTrackInstanceParam, 1>(
          *parameter.TextureUVTypeParameterPtr, instances, verteies,
          parameter.SplineDivision, global_scale);
    }

    TrailRendererUtils::AssignUVs<VERTEX, efkTrackInstanceParam, 2>(
        *parameter.TextureUVTypeParameterPtr, instances, verteies,
        parameter.SplineDivision, global_scale);
    TrailRendererUtils::AssignUVs<VERTEX, efkTrackInstanceParam, 3>(
        *parameter.TextureUVTypeParameterPtr, instances, verteies,
        parameter.SplineDivision, global_scale);
    TrailRendererUtils::AssignUVs<VERTEX, efkTrackInstanceParam, 4>(
        *parameter.TextureUVTypeParameterPtr, instances, verteies,
        parameter.SplineDivision, global_scale);
    TrailRendererUtils::AssignUVs<VERTEX, efkTrackInstanceParam, 5>(
        *parameter.TextureUVTypeParameterPtr, instances, verteies,
        parameter.SplineDivision, global_scale);
    TrailRendererUtils::AssignUVs<VERTEX, efkTrackInstanceParam, 6>(
        *parameter.TextureUVTypeParameterPtr, instances, verteies,
        parameter.SplineDivision, global_scale);
      ...
      for (size_t loop = 0; loop < instances.size() - 1; loop++) {
        ...
        for (int32_t sploop = 0; sploop < parameter.SplineDivision; sploop++) {
          for (size_t i = 0; i < 8; i++) {
            ...
            memcpy(c, param.CustomData1.data(),
    ...
    if (customData2Count_ > 0) {
      ...
      for (size_t loop = 0; loop < instances.size() - 1; loop++) {
        ...
        for (int32_t sploop = 0; sploop < parameter.SplineDivision; sploop++) {
          for (size_t i = 0; i < 8; i++) {
            auto c = (float *)(&custom[0]);
            memcpy(c, param.CustomData2.data(),
...
public:
  TrackRendererBase(RENDERER *renderer)
      : renderer_(renderer), ribbonCount_(0), ringBufferOffset_(0),
...
public:
  void Rendering(const efkTrackNodeParam &parameter,
                 const efkTrackInstanceParam &instanceParameter,
                 void *userData) override {
    Rendering_(parameter, instanceParameter, renderer_->GetCameraMatrix());
  }

  void BeginRenderingGroup(const efkTrackNodeParam &param, int32_t count,
                           void *userData) override {
    ribbonCount_ = 0;
    ...
    renderer_->GetStandardRenderer()->BeginRenderingAndRenderingIfRequired(
        state, vertexCount, stride_, (void *&)ringBufferData_);
    vertexCount_ = vertexCount;
  }

  void EndRendering(const efkTrackNodeParam &parameter,
                    void *userData) override {
