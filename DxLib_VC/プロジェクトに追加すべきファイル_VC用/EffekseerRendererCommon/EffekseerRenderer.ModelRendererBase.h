...
template <int MODEL_COUNT> struct ModelRendererVertexConstantBuffer {
  ...
  void SetModelFlipbookParameter(
      const Effekseer::NodeRendererFlipbookParameter &param) {}

  void SetModelAlphaUV(int32_t index, float x, float y, float w, float h) {}

  void SetModelUVDistortionUV(int32_t index, float x, float y, float w,
                              float h) {}
  ...
  void SetModelBlendUVDistortionUV(int32_t index, float x, float y, float w,
                                   float h) {}
...
template <int MODEL_COUNT> struct ModelRendererAdvancedVertexConstantBuffer {
  Effekseer::Matrix44 CameraMatrix;
  Effekseer::Matrix44 ModelMatrix[MODEL_COUNT];
  float ModelUV[MODEL_COUNT][4];
...
template <int MODEL_COUNT> struct ModelRendererMaterialVertexConstantBuffer {
  ...
  void SetModelBlendAlphaUV(int32_t iondex, float x, float y, float w,
                            float h) {}

  void SetModelBlendUVDistortionUV(int32_t iondex, float x, float y, float w,
                                   float h) {}
...
class ModelRendererBase
    : public ::Effekseer::ModelRenderer,
      public ::Effekseer::SIMD::AlignedAllocationPolicy<16> {
protected:
  struct KeyValue {
    float Key;
    int Value;
  ...
  void VectorToFloat4(const ::Effekseer::SIMD::Vec3f &v,
                      std::array<float, 4> &fc) {
  ...
  void GetInversedFlags(RENDERER *renderer, std::array<float, 4> &uvInversed,
  ...
  void SortTemporaryValues(RENDERER *renderer, const efkModelNodeParam &param) {
    if (param.DepthParameterPtr->ZSort != Effekseer::ZSortType::None) {
      ...
      for (size_t i = 0; i < keyValues_.size(); i++) {
        efkVector3D t(matrixes_[i].Values[3][0], matrixes_[i].Values[3][1],
                      matrixes_[i].Values[3][2]);
      ...
      if (param.DepthParameterPtr->ZSort == Effekseer::ZSortType::NormalOrder) {
        std::sort(keyValues_.begin(), keyValues_.end(),
                  [](const KeyValue &a, const KeyValue &b) -> bool {
                    return a.Key < b.Key;
                  });
      } else {
        std::sort(keyValues_.begin(), keyValues_.end(),
      ...
      for (size_t i = 0; i < keyValues_.size(); i++) {
        ...
        blendUVDistortionUVSorted_[keyValues_[i].Value] =
            blendUvDistortionUv_[i];
        flipbookIndexAndNextRateSorted_[keyValues_[i].Value] =
            flipbookIndexAndNextRate_[i];
  ...
  void StoreFileUniform(RENDERER *renderer, SHADER *shader_,
                        Effekseer::MaterialRef material,
                        Effekseer::MaterialRenderData *materialRenderData,
                        const efkModelNodeParam &param, int32_t renderPassInd,
                        float *&cutomData1Ptr, float *&cutomData2Ptr) {
    std::array<float, 4> uvInversed;
    std::array<float, 4> uvInversedBack;
    cutomData1Ptr = nullptr;
    cutomData2Ptr = nullptr;

    const auto materialGradientCount = static_cast<int32_t>(
        Effekseer::Min(materialRenderData->MaterialGradients.size(),
                       Effekseer::UserGradientSlotMax));
    ...
    predefined_uniforms[2] =
        renderer->GetImpl()->MaintainGammaColorInLinearColorSpace ? 1.0f : 0.0f;
    predefined_uniforms[3] = param.LocalTime;

    // vs
    int32_t vsOffset =
        sizeof(Effekseer::Matrix44) +
        (sizeof(Effekseer::Matrix44) + sizeof(float) * 4 * 3) * InstanceCount;

    renderer->SetVertexBufferToShader(uvInversedMaterial.data(),
                                      sizeof(float) * 4, vsOffset);
    vsOffset += (sizeof(float) * 4);

    renderer->SetVertexBufferToShader(predefined_uniforms.data(),
                                      sizeof(float) * 4, vsOffset);
    vsOffset += (sizeof(float) * 4);

    renderer->SetVertexBufferToShader(cameraPosition, sizeof(float) * 4,
                                      vsOffset);
    vsOffset += (sizeof(float) * 4);
    ...
    if (customData1Count_ > 0) {
      cutomData1Ptr =
          (float *)((uint8_t *)shader_->GetVertexConstantBuffer() + vsOffset);
    ...
    for (size_t i = 0; i < materialRenderData->MaterialUniforms.size(); i++) {
      renderer->SetVertexBufferToShader(
          materialRenderData->MaterialUniforms[i].data(), sizeof(float) * 4,
          vsOffset);
      vsOffset += (sizeof(float) * 4);
    }
    ...
    for (size_t i = 0; i < materialGradientCount; i++) {
      ...
      renderer->SetVertexBufferToShader(data.data(), sizeof(float) * 4 * 13,
                                        vsOffset);
    ...
    // ps
    int32_t psOffset = 0;
    renderer->SetPixelBufferToShader(uvInversedMaterial.data(),
                                     sizeof(float) * 4, psOffset);
    psOffset += (sizeof(float) * 4);

    renderer->SetPixelBufferToShader(predefined_uniforms.data(),
                                     sizeof(float) * 4, psOffset);
    psOffset += (sizeof(float) * 4);

    renderer->SetPixelBufferToShader(cameraPosition, sizeof(float) * 4,
                                     psOffset);
    psOffset += (sizeof(float) * 4);
    ...
    softParticleParam.SetParam(0.0f, 0.0f, 0.0f, param.Maginification,
    ...
    renderer->SetPixelBufferToShader(
        softParticleParam.reconstructionParam1.data(), sizeof(float) * 4,
        psOffset);
    psOffset += (sizeof(float) * 4);

    renderer->SetPixelBufferToShader(
        softParticleParam.reconstructionParam2.data(), sizeof(float) * 4,
        psOffset);
    psOffset += (sizeof(float) * 4);

    // shader model
    material =
        param.EffectPointer->GetMaterial(materialRenderData->MaterialIndex);
    ...
    renderer->SetPixelBufferToShader(lightDirection, sizeof(float) * 4,
                                     psOffset);
    psOffset += (sizeof(float) * 4);
    ...
    renderer->SetPixelBufferToShader(lightAmbientColor, sizeof(float) * 4,
                                     psOffset);
    psOffset += (sizeof(float) * 4);
    ...
    for (int32_t i = 0; i < materialGradientCount; i++) {
      auto data = ToUniform(*materialRenderData->MaterialGradients[i]);
      renderer->SetPixelBufferToShader(data.data(), sizeof(float) * 4 * 13,
                                       psOffset);
  ...
  template <typename RENDERER, typename SHADER, int InstanceCount,
            typename VertexConstantBufferType, bool REQUIRE_ADVANCED_DATA,
            bool DISTORTION>
  void StoreFixedUniforms(RENDERER *renderer, SHADER *shader_,
                          const efkModelNodeParam &param) {
    VertexConstantBufferType *vcb =
        (VertexConstantBufferType *)shader_->GetVertexConstantBuffer();
    std::array<float, 4> uvInversed;
    std::array<float, 4> uvInversedBack;
    ...
    if (DISTORTION) {
      auto pcb =
          (PixelConstantBufferDistortion *)shader_->GetPixelConstantBuffer();
      pcb->DistortionIntencity[0] =
          param.BasicParameterPtr->DistortionIntensity;

      pcb->UVInversedBack[0] = uvInversedBack[0];
      pcb->UVInversedBack[1] = uvInversedBack[1];

      pcb->FlipbookParam.EnableInterpolation = static_cast<float>(
          param.BasicParameterPtr->Flipbook.EnableInterpolation);
      pcb->FlipbookParam.InterpolationType = static_cast<float>(
          param.BasicParameterPtr->Flipbook.InterpolationType);

      pcb->UVDistortionParam.Intensity =
          param.BasicParameterPtr->UVDistortionIntensity;
      pcb->UVDistortionParam.BlendIntensity =
          param.BasicParameterPtr->BlendUVDistortionIntensity;
      pcb->UVDistortionParam.UVInversed[0] = uvInversed[0];
      pcb->UVDistortionParam.UVInversed[1] = uvInversed[1];

      pcb->BlendTextureParam.BlendType =
          static_cast<float>(param.BasicParameterPtr->TextureBlendType);
      ...
      pcb->SoftParticleParam.SetParam(
          ...
          param.Maginification, reconstructionParam.DepthBufferScale,
          reconstructionParam.DepthBufferOffset,
          reconstructionParam.ProjectionMatrix33,
          reconstructionParam.ProjectionMatrix34,
          reconstructionParam.ProjectionMatrix43,
      ...
      pcb->MiscFlags[0] =
          renderer->GetImpl()->MaintainGammaColorInLinearColorSpace ? 1.0f
                                                                    : 0.0f;

      // specify predefined parameters
      if (param.BasicParameterPtr->MaterialType ==
          Effekseer::RendererMaterialType::Lighting) {
...

      {
        ...
        pcb->LightAmbientColor =
            ColorToFloat4(renderer->GetLightAmbientColor());
      }

      pcb->SetEmissiveScaling(
          static_cast<float>(param.BasicParameterPtr->EmissiveScaling));

      if (REQUIRE_ADVANCED_DATA) {
        pcb->SetModelFlipbookParameter(
            param.BasicParameterPtr->Flipbook.EnableInterpolation,
            static_cast<float>(
                param.BasicParameterPtr->Flipbook.InterpolationType));
        pcb->SetModelUVDistortionParameter(
            param.BasicParameterPtr->UVDistortionIntensity,
            param.BasicParameterPtr->BlendUVDistortionIntensity,
            {uvInversed[0], uvInversed[1]});
        pcb->SetModelBlendTextureParameter(
            static_cast<float>(param.BasicParameterPtr->TextureBlendType));
        ...
        pcb->SetCameraFrontDirection(-CameraFront.X, -CameraFront.Y,
                                     -CameraFront.Z);
        ...
        pcb->SetEdgeParameter(
            ColorToFloat4(
                Effekseer::Color(param.BasicParameterPtr->EdgeColor[0],
...
public:
  ...
  template <typename RENDERER>
  void BeginRendering_(RENDERER *renderer, const efkModelNodeParam &parameter,
    ...
    if (parameter.BasicParameterPtr->MaterialType ==
            ::Effekseer::RendererMaterialType::File &&
        parameter.BasicParameterPtr->MaterialRenderDataPtr != nullptr &&
        parameter.BasicParameterPtr->MaterialRenderDataPtr->MaterialIndex >=
            0 &&
        parameter.EffectPointer->GetMaterial(
            parameter.BasicParameterPtr->MaterialRenderDataPtr
                ->MaterialIndex) != nullptr) {
      auto material = parameter.EffectPointer->GetMaterial(
          parameter.BasicParameterPtr->MaterialRenderDataPtr->MaterialIndex);
    ...
    collector_.Collect(renderer, parameter.EffectPointer,
                       parameter.BasicParameterPtr, parameter.EnableFalloff,
                       renderer->GetImpl()->isSoftParticleEnabled);
  }

  template <typename RENDERER>
  void Rendering_(RENDERER *renderer, const efkModelNodeParam &parameter,
                  const efkModelInstanceParam &instanceParameter,
                  void *userData) {
    ...
    if (btype == ::Effekseer::BillboardType::Fixed) {
      ...
      CalcBillboard(btype, mat43, s, R, F, baseMatrix,
                    renderer->GetCameraFrontDirection(),
                    instanceParameter.Direction);

      mat44 = ::Effekseer::SIMD::Mat43f::Scaling(s) * mat43;
    }

    if (parameter.Magnification != 1.0f) {
      mat44 = Effekseer::SIMD::Mat44f::Scaling(
                  ::Effekseer::SIMD::Vec3f(parameter.Magnification)) *
              mat44;
    }
    ...
    flipbookIndexAndNextRate_.push_back(
        instanceParameter.FlipbookIndexAndNextRate);
    ...
    particleTimes_.push_back({instanceParameter.ParticleTimes[0],
                              instanceParameter.ParticleTimes[1]});
  ...
  template <typename RENDERER, typename SHADER, typename MODEL, bool Instancing,
            int InstanceCount>
  void EndRendering_(RENDERER *renderer, SHADER *advanced_shader_lit,
                     SHADER *advanced_shader_unlit,
                     SHADER *advanced_shader_distortion, SHADER *shader_lit,
                     SHADER *shader_unlit, SHADER *shader_distortion,
                     const efkModelNodeParam &param, void *userData) {
    if (matrixes_.size() == 0)
      return;
    ...
    if (renderer->GetExternalShaderSettings() == nullptr) {
      ...
      shader_unlit->OverrideShader(
          renderer->GetExternalShaderSettings()->ModelShader);
    }

    int32_t renderPassCount = 1;

    if (param.BasicParameterPtr->MaterialRenderDataPtr != nullptr &&
        param.BasicParameterPtr->MaterialRenderDataPtr->MaterialIndex >= 0) {
      auto material = param.EffectPointer->GetMaterial(
          param.BasicParameterPtr->MaterialRenderDataPtr->MaterialIndex);
    ...
    for (int32_t renderPassInd = 0; renderPassInd < renderPassCount;
         renderPassInd++) {
      Effekseer::MaterialRenderData *materialRenderData =
          param.BasicParameterPtr->MaterialRenderDataPtr;

      if (materialRenderData != nullptr &&
          materialRenderData->MaterialIndex >= 0 &&
          param.EffectPointer->GetMaterial(materialRenderData->MaterialIndex) !=
              nullptr) {
        RenderPass<RENDERER, SHADER, MODEL, Instancing, InstanceCount,
                   ModelRendererMaterialVertexConstantBuffer<InstanceCount>,
                   false>(renderer, advanced_shader_lit, advanced_shader_unlit,
                          advanced_shader_distortion, shader_lit, shader_unlit,
                          shader_distortion, param, renderPassInd, userData);
      } else {
        if (collector_.DoRequireAdvancedRenderer()) {
          RenderPass<RENDERER, SHADER, MODEL, Instancing, InstanceCount,
                     ModelRendererAdvancedVertexConstantBuffer<InstanceCount>,
                     true>(renderer, advanced_shader_lit, advanced_shader_unlit,
                           advanced_shader_distortion, shader_lit, shader_unlit,
                           shader_distortion, param, renderPassInd, userData);
        } else {
          RenderPass<RENDERER, SHADER, MODEL, Instancing, InstanceCount,
                     ModelRendererVertexConstantBuffer<InstanceCount>, false>(
              renderer, advanced_shader_lit, advanced_shader_unlit,
              advanced_shader_distortion, shader_lit, shader_unlit,
              shader_distortion, param, renderPassInd, userData);
        }
      }
  ...
  template <typename RENDERER, typename SHADER, typename MODEL, bool Instancing,
            int InstanceCount, typename VertexConstantBufferType,
            bool REQUIRE_ADVANCED_DATA>
  void RenderPass(RENDERER *renderer, SHADER *advanced_shader_lit,
                  SHADER *advanced_shader_unlit,
                  SHADER *advanced_shader_distortion, SHADER *shader_lit,
                  SHADER *shader_unlit, SHADER *shader_distortion,
                  const efkModelNodeParam &param, int32_t renderPassInd,
    ...
    auto isBackgroundRequired =
        collector_.IsBackgroundRequiredOnFirstPass && renderPassInd == 0;
    ...
    if (isBackgroundRequired) {
      collector_.Textures[collector_.BackgroundIndex] =
    ...
    if (collector_.IsDepthRequired) {
      if (depthTexture == nullptr ||
          (param.BasicParameterPtr->SoftParticleDistanceFar == 0.0f &&
        ...
        depthTexture = renderer->GetImpl()->GetProxyTexture(
            EffekseerRenderer::ProxyTextureType::White);
      }
    ...
    renderer->SetTextures(shader_, collector_.Textures.data(),
                          collector_.TextureCount);
    ...
    if (materialRenderData != nullptr && material != nullptr) {
      StoreFileUniform<RENDERER, SHADER, InstanceCount>(
          renderer, shader_, material, materialRenderData, param, renderPassInd,
          cutomData1Ptr, cutomData2Ptr);

      vcb->CameraMatrix = renderer->GetCameraProjectionMatrix();
    } else {
      if (distortion) {
        StoreFixedUniforms<RENDERER, SHADER, InstanceCount,
                           VertexConstantBufferType, REQUIRE_ADVANCED_DATA,
                           true>(renderer, shader_, param);
      } else {
        StoreFixedUniforms<RENDERER, SHADER, InstanceCount,
                           VertexConstantBufferType, REQUIRE_ADVANCED_DATA,
                           false>(renderer, shader_, param);
      }
    }
    ...
    if (Instancing && isTimeSame) {
      ...
      renderer->SetVertexBuffer(model->GetVertexBuffer(stTime0),
                                sizeof(Effekseer::Model::Vertex));
      ...
      for (size_t loop = 0; loop < matrixes_.size();) {
        int32_t modelCount = Effekseer::Min(
            static_cast<int32_t>(matrixes_.size()) - (int32_t)loop,
            InstanceCount);
        ...
        for (int32_t num = 0; num < modelCount; num++) {
          ...
          if (param.EnableViewOffset) {
            ApplyViewOffset(modelMatrix, renderer->GetCameraMatrix(),
                            viewOffsetDistance_[loop + num]);
          }

          ApplyDepthParameters(modelMatrix, renderer->GetCameraFrontDirection(),
                               renderer->GetCameraPosition(),
                               param.DepthParameterPtr, param.IsRightHand);
          ...
          vcb->SetModelAlphaUV(
              num, alphaUv_[loop + num].X, alphaUv_[loop + num].Y,
              alphaUv_[loop + num].Width, alphaUv_[loop + num].Height);
          vcb->SetModelUVDistortionUV(num, uvDistortionUv_[loop + num].X,
                                      uvDistortionUv_[loop + num].Y,
                                      uvDistortionUv_[loop + num].Width,
                                      uvDistortionUv_[loop + num].Height);
          vcb->SetModelBlendUV(
              num, blendUv_[loop + num].X, blendUv_[loop + num].Y,
              blendUv_[loop + num].Width, blendUv_[loop + num].Height);
          vcb->SetModelBlendAlphaUV(num, blendAlphaUv_[loop + num].X,
                                    blendAlphaUv_[loop + num].Y,
                                    blendAlphaUv_[loop + num].Width,
                                    blendAlphaUv_[loop + num].Height);
          vcb->SetModelBlendUVDistortionUV(
              num, blendUvDistortionUv_[loop + num].X,
              blendUvDistortionUv_[loop + num].Y,
              blendUvDistortionUv_[loop + num].Width,
              blendUvDistortionUv_[loop + num].Height);
          vcb->SetModelFlipbookIndexAndNextRate(
              num, flipbookIndexAndNextRate_[loop + num]);
          vcb->SetModelAlphaThreshold(num, alphaThreshold_[loop + num]);
          vcb->SetModelParticleTime(num, particleTimes_[loop + num][0],
                                    particleTimes_[loop + num][1]);
        ...
        if (VertexType == ModelRendererVertexType::Instancing) {
          renderer->DrawPolygonInstanced(
              model->GetVertexCount(stTime0),
              model->GetFaceCount(stTime0) * indexPerFace, modelCount);
        } else {
          assert(0);
        }
      ...
      for (size_t loop = 0; loop < matrixes_.size();) {
        ...
        // Invalid unless layout is set after buffer
        renderer->SetVertexBuffer(model->GetVertexBuffer(stTime),
                                  sizeof(Effekseer::Model::Vertex));
        ...
        vcb->SetModelParticleTime(0, particleTimes_[loop][0],
                                  particleTimes_[loop][1]);

        vcb->SetModelAlphaUV(0, alphaUv_[loop].X, alphaUv_[loop].Y,
                             alphaUv_[loop].Width, alphaUv_[loop].Height);
        vcb->SetModelUVDistortionUV(
            0, uvDistortionUv_[loop].X, uvDistortionUv_[loop].Y,
            uvDistortionUv_[loop].Width, uvDistortionUv_[loop].Height);
        vcb->SetModelBlendUV(0, blendUv_[loop].X, blendUv_[loop].Y,
                             blendUv_[loop].Width, blendUv_[loop].Height);
        vcb->SetModelBlendAlphaUV(
            0, blendAlphaUv_[loop].X, blendAlphaUv_[loop].Y,
            blendAlphaUv_[loop].Width, blendAlphaUv_[loop].Height);
        vcb->SetModelBlendUVDistortionUV(0, blendUvDistortionUv_[loop].X,
                                         blendUvDistortionUv_[loop].Y,
                                         blendUvDistortionUv_[loop].Width,
                                         blendUvDistortionUv_[loop].Height);
        vcb->SetModelFlipbookIndexAndNextRate(0,
                                              flipbookIndexAndNextRate_[loop]);
        ...
        shader_->SetConstantBuffer();
        renderer->DrawPolygon(model->GetVertexCount(stTime),
                              model->GetFaceCount(stTime) * indexPerFace);
