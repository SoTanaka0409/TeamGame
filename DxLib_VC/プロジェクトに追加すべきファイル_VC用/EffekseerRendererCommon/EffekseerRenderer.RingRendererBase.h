...
class RingRendererBase : public ::Effekseer::RingRenderer,
...
protected:
  struct RingRenderState {
    ...
    RingRenderState(const ::Effekseer::SIMD::Mat43f &mat43, float outerHeight_,
                    float innerHeight_, float centerHeight_)
  ...
  ::Effekseer::RendererMaterialType materialType_ =
...
public:
  RingRendererBase(RENDERER *renderer)
      : renderer_(renderer), ringBufferOffset_(0), ringBufferData_(nullptr),
        spriteCount_(0), instanceCount_(0) {}
...
protected:
  void RenderingInstance(const efkRingInstanceParam &instanceParameter,
    ...
    if (collector.ShaderType == RendererShaderType::Material) {
      Rendering_Internal<DynamicVertex, FLIP_RGB_FLAG>(
          parameter, instanceParameter, camera);
    } else if (collector.ShaderType == RendererShaderType::AdvancedLit) {
      Rendering_Internal<AdvancedLightingVertex, FLIP_RGB_FLAG>(
          parameter, instanceParameter, camera);
    } else if (collector.ShaderType ==
               RendererShaderType::AdvancedBackDistortion) {
      Rendering_Internal<AdvancedLightingVertex, FLIP_RGB_FLAG>(
          parameter, instanceParameter, camera);
    } else if (collector.ShaderType == RendererShaderType::AdvancedUnlit) {
      Rendering_Internal<AdvancedSimpleVertex, FLIP_RGB_FLAG>(
          parameter, instanceParameter, camera);
    } else if (collector.ShaderType == RendererShaderType::Lit) {
      Rendering_Internal<LightingVertex, FLIP_RGB_FLAG>(
          parameter, instanceParameter, camera);
    } else if (collector.ShaderType == RendererShaderType::BackDistortion) {
      Rendering_Internal<LightingVertex, FLIP_RGB_FLAG>(
          parameter, instanceParameter, camera);
    } else {
      Rendering_Internal<SimpleVertex, FLIP_RGB_FLAG>(
          parameter, instanceParameter, camera);
    }
  }

  void BeginRendering_(RENDERER *renderer, int32_t count,
                       const efkRingNodeParam &param, void *userData) {
    spriteCount_ = 0;
    const auto singleVertexCount = param.VertexCount * 8;
    const auto singleSpriteCount = param.VertexCount * 2;

    count =
        (std::min)(count, renderer->GetSquareMaxCount() / singleSpriteCount);
    ...
    state.Distortion = param.BasicParameterPtr->MaterialType ==
    ...
    state.CopyMaterialFromParameterToState(renderer_, param.EffectPointer,
    ...
    renderer->GetStandardRenderer()->BeginRenderingAndRenderingIfRequired(
        state, count * singleVertexCount, stride_, (void *&)ringBufferData_);
  ...
  void PrepareMatrix(const efkRingNodeParam &parameter,
    ...
    if (parameter.Billboard != ::Effekseer::BillboardType::Fixed) {
      ...
      if (parameter.EnableViewOffset) {
        ...
        CalcBillboard(parameter.Billboard, mat43, s, R, F, instMat,
                      renderer_->GetCameraFrontDirection(),
                      instanceParameter.Direction);
      } else {
        CalcBillboard(
            parameter.Billboard, mat43, s, R, F, instanceParameter.SRTMatrix43,
            renderer_->GetCameraFrontDirection(), instanceParameter.Direction);
      }

      ApplyDepthParameters(mat43, renderer_->GetCameraFrontDirection(),
                           renderer_->GetCameraPosition(), s,
                           parameter.DepthParameterPtr, parameter.IsRightHand);
  ...
  RingRenderState
  InitializeRingState(const efkRingNodeParam &parameter,
                      const efkRingInstanceParam &instanceParameter,
                      const ::Effekseer::SIMD::Mat43f &mat43) {
    const int32_t singleVertexCount = parameter.VertexCount * 8;
    const float vertexCount =
        Effekseer::AvoidZero(static_cast<float>(parameter.VertexCount));
    const float invVertexCount = 1.0f / vertexCount;

    const float circleAngleDegree = (instanceParameter.ViewingAngleEnd -
    ...
    state.advancedUVv2[0] =
        state.advancedUVv1[0] + instanceParameter.AlphaUV.Height * 0.5f;
    state.advancedUVv2[1] =
        state.advancedUVv1[1] + instanceParameter.UVDistortionUV.Height * 0.5f;
    state.advancedUVv2[2] =
        state.advancedUVv1[2] + instanceParameter.BlendUV.Height * 0.5f;
    state.advancedUVv2[3] =
        state.advancedUVv1[3] + instanceParameter.BlendAlphaUV.Height * 0.5f;
    state.advancedUVv2[4] = state.advancedUVv1[4] +
                            instanceParameter.BlendUVDistortionUV.Height * 0.5f;

    state.advancedUVv3[0] =
        state.advancedUVv1[0] + instanceParameter.AlphaUV.Height;
    state.advancedUVv3[1] =
        state.advancedUVv1[1] + instanceParameter.UVDistortionUV.Height;
    state.advancedUVv3[2] =
        state.advancedUVv1[2] + instanceParameter.BlendUV.Height;
    state.advancedUVv3[3] =
        state.advancedUVv1[3] + instanceParameter.BlendAlphaUV.Height;
    state.advancedUVv3[4] =
        state.advancedUVv1[4] + instanceParameter.BlendUVDistortionUV.Height;
    ...
    state.outerCurrent = ::Effekseer::SIMD::Vec3f(
        state.cos_ * state.outerRadius, state.sin_ * state.outerRadius, 0.0f);
    state.innerCurrent = ::Effekseer::SIMD::Vec3f(
        state.cos_ * state.innerRadius, state.sin_ * state.innerRadius, 0.0f);
    state.centerCurrent = ::Effekseer::SIMD::Vec3f(
        state.cos_ * state.centerRadius, state.sin_ * state.centerRadius, 0.0f);
  ...
  void RenderRingVertices(const efkRingNodeParam &parameter,
    ...
    StrideView<VERTEX> verteies(ringBufferData_, stride_,
                                state.singleVertexCount);
    ...
    for (int i = 0; i < state.singleVertexCount; i += 8) {
      ...
      outerNext = ::Effekseer::SIMD::Vec3f{state.cos_ * state.outerRadius,
                                           state.sin_ * state.outerRadius, 0};
      innerNext = ::Effekseer::SIMD::Vec3f{state.cos_ * state.innerRadius,
                                           state.sin_ * state.innerRadius, 0};
      centerNext = ::Effekseer::SIMD::Vec3f{state.cos_ * state.centerRadius,
                                            state.sin_ * state.centerRadius, 0};
      ...
      state.currentAngleDegree =
          Effekseer::Min(state.currentAngleDegree, state.circleAngleDegree);
      float alpha = 1.0f;
      if (state.currentAngleDegree < state.fadeStartAngle) {
        alpha = state.currentAngleDegree / state.fadeStartAngle;
      } else if (state.currentAngleDegree >
                 state.circleAngleDegree - state.fadeEndingAngle) {
        alpha = 1.0f - (state.currentAngleDegree -
                        (state.circleAngleDegree - state.fadeEndingAngle)) /
                           state.fadeEndingAngle;
      }
      ...
      for (int32_t uvi = 0; uvi < RingRenderState::AdvancedUVNum; uvi++) {
        advancedUVtexNext[uvi] =
            state.advancedUVCurrent[uvi] + state.advancedUVStep[uvi];
      }
      ...
      if (VertexNormalRequired<VERTEX>()) {
        ...
        ::Effekseer::SIMD::Vec3f outerBefore{c_b * state.outerRadius,
                                             s_b * state.outerRadius, 0.0f};
        ...
        ::Effekseer::SIMD::Vec3f outerNN{c_n * state.outerRadius,
                                         s_n * state.outerRadius, 0.0f};
        state.outerTransform.Transform(outerNN);

        ::Effekseer::SIMD::Vec3f tangent0 =
            (state.outerCurrent - outerBefore).GetNormal();
        ::Effekseer::SIMD::Vec3f tangent1 =
            (outerNext - state.outerCurrent).GetNormal();
        ...
        normalCurrent =
            ::Effekseer::SIMD::Vec3f::Cross(tangentCurrent, binormalCurrent);
  ...
  void WriteCustomData(const efkRingInstanceParam &instanceParameter,
                       int32_t singleVertexCount) {
    // custom parameter
    if (customData1Count_ > 0) {
      StrideView<float> custom(ringBufferData_ + sizeof(DynamicVertex), stride_,
                               singleVertexCount);
      for (int i = 0; i < singleVertexCount; i++) {
        auto c = (float *)(&custom[i]);
        memcpy(c, instanceParameter.CustomData1.data(),
               sizeof(float) * customData1Count_);
      }
    }

    if (customData2Count_ > 0) {
      StrideView<float> custom(ringBufferData_ + sizeof(DynamicVertex) +
                                   sizeof(float) * customData1Count_,
                               stride_, singleVertexCount);
      for (int i = 0; i < singleVertexCount; i++) {
        auto c = (float *)(&custom[i]);
        memcpy(c, instanceParameter.CustomData2.data(),
               sizeof(float) * customData2Count_);
      }
    }
  }
  ...
  void EndRendering_(RENDERER *renderer, const efkRingNodeParam &param,
    ...
    if (param.DepthParameterPtr->ZSort != Effekseer::ZSortType::None) {
      for (auto &kv : instances_) {
        ...
        Effekseer::SIMD::Vec3f frontDirection =
            renderer_->GetCameraFrontDirection();
...
public:
  void BeginRendering(const efkRingNodeParam &parameter, int32_t count,
                      void *userData) {
    BeginRendering_(renderer_, count, parameter, userData);
  }

  void Rendering(const efkRingNodeParam &parameter,
                 const efkRingInstanceParam &instanceParameter,
                 void *userData) {
    if (ringBufferData_ == nullptr)
      return;
    if (spriteCount_ + 2 * parameter.VertexCount >
        renderer_->GetSquareMaxCount())
      return;
  ...
  void EndRendering(const efkRingNodeParam &parameter, void *userData) {
    if (ringBufferData_ == nullptr)
      return;
    if (spriteCount_ == 0 &&
        parameter.DepthParameterPtr->ZSort == Effekseer::ZSortType::None)
      return;
