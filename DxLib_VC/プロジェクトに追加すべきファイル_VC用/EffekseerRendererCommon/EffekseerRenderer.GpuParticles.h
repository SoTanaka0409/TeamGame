...
public:
  Resource() = default;
  ~Resource() = default;
  virtual const Effekseer::GpuParticles::ParamSet &
  GetParamSet() const override {
    return ParamSet;
  }
...
public:
  ...
  Effekseer::GpuParticles::ResourceRef
  CreateResource(const Effekseer::GpuParticles::ParamSet &paramSet,
                 const Effekseer::Effect *effect);

protected:
  GpuParticles::ParameterData
  ToParamData(const Effekseer::GpuParticles::ParamSet &paramSet);
  GpuParticles::PipelineStateKey
  ToPiplineStateKey(const Effekseer::GpuParticles::ParamSet &paramSet);
...
public:
  ...
  struct Emitter {
    ...
    bool IsAlive() const { return (Data.FlagBits & 1) != 0; }
    bool IsEmitting() const { return (Data.FlagBits & 2) != 0; }
...
public:
  ...
  virtual EmitterID
  NewEmitter(Effekseer::GpuParticles::ResourceRef resource,
             Effekseer::InstanceGlobal *instanceGlobal) override;
  ...
  virtual void SetTransform(EmitterID emitterID,
                            const Effekseer::Matrix43 &transform) override;

  virtual void SetColor(EmitterID emitterID, Effekseer::Color color) override;

  virtual void SetDeltaTime(Effekseer::InstanceGlobal *instanceGlobal,
                            float deltaTime) override;

  virtual void
  KillParticles(Effekseer::InstanceGlobal *instanceGlobal) override;

  virtual int32_t
  GetParticleCount(Effekseer::InstanceGlobal *instanceGlobal) override;

private:
  int32_t
  GetEmitterParticleCount(const Emitter &emitter,
                          const Effekseer::GpuParticles::ParamSet &paramSet);

  void FreeEmitter(EmitterID emitterID);

  GpuParticles::PipelineStateRef
  GetOrCreatePipelineState(GpuParticles::PipelineStateKey key);
...
protected:
  ...
  Effekseer::CustomVector<std::tuple<GpuParticles::PipelineStateKey,
                                     GpuParticles::PipelineStateRef>>
      pipelineParticleRenders_;
