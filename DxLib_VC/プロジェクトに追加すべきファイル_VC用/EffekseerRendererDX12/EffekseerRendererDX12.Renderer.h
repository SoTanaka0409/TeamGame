...
CreateGraphicsDevice(ID3D12Device *device, ID3D12CommandQueue *commandQueue,
...
/**
        ...
        @param	depthFormat	Rendered screen depth format. If depth doesn't
   exists, it must be DXGI_FORMAT_UNKNOWN
...
Create(::Effekseer::Backend::GraphicsDeviceRef graphicsDevice,
       DXGI_FORMAT *renderTargetFormats, int32_t renderTargetCount,
       DXGI_FORMAT depthFormat, bool isReversedDepth, int32_t squareMaxCount);
...
*/
::EffekseerRenderer::RendererRef
Create(ID3D12Device *device, ID3D12CommandQueue *commandQueue,
       int32_t swapBufferCount, DXGI_FORMAT *renderTargetFormats,
       int32_t renderTargetCount, DXGI_FORMAT depthFormat, bool isReversedDepth,
       int32_t squareMaxCount);

Effekseer::Backend::TextureRef
CreateTexture(::Effekseer::Backend::GraphicsDeviceRef graphicsDevice,
...
CommandListProperty GetCommandListProperty(
...
/**
        ...
        @note
        \~English	dx12CommandList can be null. In this case, it need to
   call ExecuteCommandList
        \~Japanese	dx12CommandList
   はnullにできる。その場合、ExecuteCommandListを呼ぶ必要がある。
*/
