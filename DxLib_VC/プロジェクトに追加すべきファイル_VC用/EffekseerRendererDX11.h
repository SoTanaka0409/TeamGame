...
public:
  ...
  */
  virtual bool OnDistorting(Renderer *renderer) { return false; }
};
...
/**
        @brief
        \~english A type of texture which is rendered when textures are not
   assigned.
...
::Effekseer::ModelLoaderRef
CreateModelLoader(::Effekseer::Backend::GraphicsDeviceRef gprahicsDevice,
...
public:
  /**
          @brief	only for Effekseer backend developer. Effekseer User
     doesn't need it.
  */
  Impl *GetImpl();
  ...
  */
  ...
  /**
          ...
          @param front (Right Hand) a direction from focus to eye, (Left Hand) a
     direction from eye to focus,
          @note
          These are set based on camera matrix automatically.
          It is failed on some platform.
  */
  virtual void SetCameraParameter(const ::Effekseer::Vector3D &front,
  ...
  */
  virtual ::Effekseer::TextureLoaderRef CreateTextureLoader(
      ::Effekseer::FileInterfaceRef fileInterface = nullptr) = 0;

  /**
          @brief	標準のモデル読込クラスを生成する。
  */
  virtual ::Effekseer::ModelLoaderRef
  CreateModelLoader(::Effekseer::FileInterfaceRef fileInterface = nullptr) = 0;
  ...
  */
  virtual ::Effekseer::MaterialLoaderRef CreateMaterialLoader(
      ::Effekseer::FileInterfaceRef fileInterface = nullptr) = 0;
  ...
  */
  ...
  /**
  @brief
  \~English	specify a command list to render.  This function is available
  except DirectX9, DirectX11 and OpenGL.
  \~Japanese
  ...
  */
  ...
  /**
          ...
          \~Japanese	背景を取得する。
          @note
          \~English	Textures are generated by a function specific to each
     backend or SetBackground.
          \~Japanese
     テクスチャは各バックエンド固有の関数かSetBackgroundで生成される。
  */
  ...
  */
  virtual ::Effekseer::Backend::TextureRef
  CreateProxyTexture(ProxyTextureType type);
  ...
  */
  ...
  /**
          @brief
          \~English	Get a depth texture and parameters to reconstruct from z
     to depth
          \~Japanese
     深度画像とZから深度を復元するためのパラメーターを取得する。
  */
  virtual void GetDepth(::Effekseer::Backend::TextureRef &texture,
                        DepthReconstructionParameter &reconstructionParam);

  /**
          @brief
          \~English	Specify a depth texture and parameters to reconstruct
     from z to depth
          \~Japanese
     深度画像とZから深度を復元するためのパラメーターを設定する。
  */
  virtual void
  SetDepth(::Effekseer::Backend::TextureRef texture,
           const DepthReconstructionParameter &reconstructionParam);

  /**
          @brief
          \~English	Specify whether maintain gamma color in a linear color
     space
          \~Japanese
  ...
  */
  ...
  /**
          ...
          For	Effekseer tools
  */
  virtual std::shared_ptr<ExternalShaderSettings>
  GetExternalShaderSettings() const;
  ...
  /**
          ...
          For	Effekseer tools
  */
  virtual void SetExternalShaderSettings(
      const std::shared_ptr<ExternalShaderSettings> &settings);
};
...
::Effekseer::Backend::GraphicsDeviceRef
CreateGraphicsDevice(ID3D11Device *device, ID3D11DeviceContext *context);

::Effekseer::MaterialLoaderRef
CreateMaterialLoader(::Effekseer::Backend::GraphicsDeviceRef graphicsDevice,
                     ::Effekseer::FileInterfaceRef fileInterface = nullptr);

::Effekseer::Backend::TextureRef
CreateTexture(::Effekseer::Backend::GraphicsDeviceRef gprahicsDevice,
  ...
  */
  ...
  /**
          @brief	\~English	Set background
                          \~Japanese	背景を設定する
  */
  [[deprecated("please use EffekseerRenderer::SetBackground")]] virtual void
  SetBackground(ID3D11ShaderResourceView *background) = 0;

  /**
          @brief	\~English	Reset internal states for
     DefferedContext
                          \~Japanese
     DefferedContextのために、内部の状態をリセットする。
          @note	\~English	Please call before BeginRendering after
     FinishCommandList
                          \~Japanese
     FinishCommandListの後でBeginRenderingの前に呼んでください。
  */
