...
protected:
  ...
  */
  virtual Buffer *ReinitializeBuffer(Buffer *cb, int32_t size) {
    return nullptr;
  }
...
struct RenderPassPipelineStateKey {
  ...
  bool operator==(const RenderPassPipelineStateKey &value) const {
    ...
    return (IsPresent == value.IsPresent && DepthFormat == value.DepthFormat &&
            IsColorCleared == value.IsColorCleared &&
            IsDepthCleared == value.IsDepthCleared &&
            SamplingCount == value.SamplingCount &&
            HasResolvedRenderTarget == value.HasResolvedRenderTarget &&
            HasResolvedDepthTarget == value.HasResolvedDepthTarget);
  }
  ...
  struct Hash {
    ...
    std::size_t operator()(const RenderPassPipelineStateKey &key) const {
      ...
      for (size_t i = 0; i < key.RenderTargetFormats.size(); i++) {
        ret += std::hash<uint64_t>()(
            static_cast<uint64_t>(key.RenderTargetFormats.at(i)));
      }
...
protected:
  ...
  bool getSize(Vec2I &size, const Texture **textures, int32_t textureCount,
               Texture *depthTexture, Texture *resolvedRenderTexture = nullptr,
...
public:
  ...
  virtual Texture *GetRenderTexture(int index) const {
    return renderTextures_.at(index);
  }

  virtual int GetRenderTextureCount() const {
    return static_cast<int32_t>(renderTextures_.size());
  }
...
public:
  ...
  /*[[deprecated("use Platform::SetWindowSize.")]]*/ virtual void
  SetWindowSize(const Vec2I &windowSize);
  ...
  */
  ...
  /**
  @brief	to prevent instances to be disposed before finish rendering,
  finish all renderings.
  */
  ...
  */
  ...
  CreateSingleFrameMemoryPool(int32_t constantBufferPoolSize,
                              int32_t drawingCount);

  /**
          @brief
          @param memoryPool if memory pool is null, allocate memory from
     graphics
  */
  virtual CommandList *CreateCommandList(SingleFrameMemoryPool *memoryPool);

  virtual RenderPass *CreateRenderPass(Texture **textures, int32_t textureCount,
                                       Texture *depthTexture) {
    return nullptr;
  }

  virtual RenderPass *CreateRenderPass(Texture *texture,
  ...
  virtual Texture *CreateTexture(const TextureParameter &parameter) {
    return nullptr;
  }

  virtual Texture *
  CreateTexture(const TextureInitializationParameter &parameter) {
    return nullptr;
  }

  virtual Texture *
  CreateRenderTexture(const RenderTextureInitializationParameter &parameter) {
    return nullptr;
  }

  virtual Texture *
  CreateDepthTexture(const DepthTextureInitializationParameter &parameter) {
    return nullptr;
  }
  ...
  */
  virtual RenderPassPipelineState *
  CreateRenderPassPipelineState(RenderPass *renderPass);
  ...
  */
  virtual RenderPassPipelineState *
  CreateRenderPassPipelineState(const RenderPassPipelineStateKey &key) {
    return nullptr;
  }

  /** For testing. Wait for all commands in queue to complete. Then read data
   * from specified render target. */
  ...
  /**
          ...
          But it is very fast. So it can call it in everyframe.
  */
  virtual Query *CreateQuery(QueryType queryType, int32_t queryCount) {
    return nullptr;
  }
  ...
  */
  virtual uint64_t TimestampToMicroseconds(uint64_t timestamp) const {
    return 0;
  }

  /**
          @brief	specify a function which is called when this instance is
     disposed.
