...
enum class ConstantBufferType {
  ...
  ShortTime, //! this constant buffer is disposed or rewrite by a frame. If
             //! shorttime, this constant buffer must be disposed by a frame.
};
...
inline Vec3F operator*(const Vec3F &v1, float v2) {
  return Vec3F(v1.X * v2, v1.Y * v2, v1.Z * v2);
}
...
inline BufferUsageType operator|(BufferUsageType lhs, BufferUsageType rhs) {
  return static_cast<BufferUsageType>(static_cast<uint32_t>(lhs) |
                                      static_cast<uint32_t>(rhs));
}
...
template <typename T> bool BitwiseContains(T value, T elm) {
  return (value & elm) == elm;
}
...
public:
  ...
  int Release() {
    ...
    bool destroy = std::atomic_fetch_sub_explicit(
                       &reference, 1, std::memory_order_consume) == 1;
    if (destroy) {
      delete this;
      return 0;
    }
...
template <typename T>
static std::shared_ptr<T> CreateSharedPtr(T *p, bool addRef = false) {
  if (addRef) {
    SafeAddRef(p);
  }
...
public:
  ...
  /**
          @brief	called when platform calls newframe, if it need to exit,
     return false
  */
